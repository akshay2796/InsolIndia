<?php include('header.php'); 
    define("PAGE_MAIN","academic_committees_detail.php");  
    define("PAGE_LIST","academic_committees.php");
...
<div class="container">
    <div class="clearfix inner_page events">
        ...
        <div class="col-md-10 col-sm-9 inner_page_right pagination__list">
            <h2>Academics Committee <span style="font-size: 18px;">(In Alphabetical Order)</span></h2>
            <div class="clearfix organisation">
                    <?php 
                   /* $SQL1  = ""; 
                    $SQL1 .= " SELECT COUNT(*) AS CT FROM " . COMMITTEES_TBL . " as G "; 
                    $SQL1 .= " WHERE status = 'ACTIVE' and committee_name !=''  ";
                    $SQL1 .= " $search "; */
                    ...
                    $SQL = "";
                    $SQL .= " SELECT * ";
                    $SQL .= " FROM ".COMMITTEES_TBL;
                    $SQL .= " WHERE status = 'ACTIVE' and committee_name !='' ";
                    $SQL .= " $search ";
                    $SQL .= " order by committee_name, position ";
                    //echo $SQL;
                        ...
                        <?php
                            ...
                            { 
                                foreach($rsLIST as $rLIST)
                                {
                                    ...
                                    $masterNAME = htmlentities(stripslashes($rLIST['committee_name']));
                                    $masterTYPE = htmlentities(stripslashes($rLIST['committee_type']));
                                    $masterPOST = htmlentities(stripslashes($rLIST['committee_post']));
                                    $masterPROFESSION= htmlentities(stripslashes($rLIST['committee_profession']));
                                    $alt_text = $masterNAME;
                                    ...
                                    $R200_IMG_EXIST = chkImageExists(MODULE_UPLOADIFY_ROOT . FLD_COMMITTEES. "/R200-" . $masterIMG);
                                    
                                    if( intval($R200_IMG_EXIST) == intval(1) )
                                    {
                                        $DISPLAY_IMG = MODULE_FILE_FOLDER . FLD_COMMITTEES. "/R200-" . $masterIMG;
                                    }
                                    else
                                    {
