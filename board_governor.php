<?php include('header.php'); 
    define("PAGE_MAIN","board_governor_detail.php");
    define("PAGE_LIST","board_governer.php");
...
<div class="container">
    <div class="clearfix inner_page events">
        ...
        <div class="col-md-10 col-sm-9 inner_page_right pagination__list">
			<h2>Board Of Governors <span style="font-size: 18px;">(In Alphabetical Order)</span></h2>
            <div class="clearfix organisation">
                    <?php 
                  /*  $SQL1  = ""; 
                    $SQL1 .= " SELECT COUNT(*) AS CT FROM " . BOARD_GOVERNORS_TBL . " as G "; 
                    $SQL1 .= " WHERE status = 'ACTIVE' and governor_name !=''  ";
                    $SQL1 .= " $search "; */
                    ...
                    $SQL = "";
                    $SQL .= " SELECT * ";
                    $SQL .= " FROM ".BOARD_GOVERNORS_TBL;
                    $SQL .= " WHERE status = 'ACTIVE' and governor_name !='' ";
                    $SQL .= " $search ";
                    $SQL .= " order by governor_name, position ";
                    //echo $SQL;
                        ...
                        <?php
                            ...
                            { 
                                foreach($rsLIST as $rLIST)
                                {
                                    ...
                                    $masterID = intval($rLIST['governor_id']);
                                    $masterNAME = htmlentities(stripslashes($rLIST['governor_name']));
                                    $masterPOST = htmlentities(stripslashes($rLIST['governor_post']));
                                    $masterPROFESSION= htmlentities(stripslashes($rLIST['governor_profession']));
                                    $alt_text = $masterNAME;
                                    ...
                                    $R200_IMG_EXIST = chkImageExists(MODULE_UPLOADIFY_ROOT . FLD_BOARD_GOVERNORS. "/R200-" . $masterIMG);
                                    
                                    if( intval($R200_IMG_EXIST) == intval(1) )
                                    {
                                        $DISPLAY_IMG = MODULE_FILE_FOLDER . FLD_BOARD_GOVERNORS. "/R200-" . $masterIMG;
                                    }
                                    else
                                    {
