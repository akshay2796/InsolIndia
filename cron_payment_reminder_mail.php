...
foreach ($row as $rs) {
    ...
    $MAIL_BODY .= '<tr>
                                                        <td valign="top" style="height: 250px; color: #333; text-align: left; font-size: 13px;">
                                                                ...
                                                                <p>As per our records your membership is about to expire and therefore request to kindly make the necessary renewal payment.</p>
...
$stmt2 = $dCON->prepare($SQL2);
$stmt2->bindParam(":membership_expired_date", $membership_expired_date);
$stmt2->bindParam(":current_date", $todaydate);
$stmt2->execute();
$row = $stmt2->fetchAll();
...
foreach ($row as $rs) {
    ...
    //$MAIL_BCC = "";
    $MAIL_BCC = $_REQUEST["emailId"];

    $MAIL_FROM = "info@sabsoftzone.in";
    $MAIL_FROMNAME = "Testing InsolIndia Payment Reminder";
