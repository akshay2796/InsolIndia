...
$.extend(Datepicker.prototype, {
	...
	defaultRenderer: {
		...
		picker: '<div class="datepick" style="border:0px solid #000;">' +
		'<div class="datepick-nav">{link:prev}{link:today}{link:next}</div>{months}' +
		'{popup:start}<div class="datepick-ctrl">{link:close}</div>{popup:end}' +
		'<div class="datepick-clear-fix"></div></div>',
		// One row of months: '{months}' to insert calendar months
		monthRow: '<div class="datepick-month-row" style="border:0px solid #000;">{months}</div>',
	...
	_autoSize: function(target, inst) {
		if (inst.get('autoSize') && !inst.inline) {
			...
			if (dateFormat.match(/[DM]/)) {
				...
				date.setMonth(findMax(inst.get(dateFormat.match(/DD/) ? // Longest month
					'monthNames' : 'monthNamesShort')));
				date.setDate(findMax(inst.get(dateFormat.match(/MM/) ? // Longest day
	...
	_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader) {
		...
		var selector = '<select class="d_month ' + this._monthYearClass +
		...
		if (yearRange == 'any') {
			selector = '<select class="d_year ' + this._monthYearClass + ' ' + this._anyYearClass +
			...
			selector = '<select  class="d_year ' + this._monthYearClass +
				'" title="' + inst.get('yearStatus') + '">';
