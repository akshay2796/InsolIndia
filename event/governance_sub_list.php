...
ob_start();
include('header.php'); 
define("PAGE_MAIN","governance_sub_detail.php");
$type_url_key = trustme($_REQUEST['master_url_key']);
$url_key = trustme($_REQUEST['url_key']);
$rsDET = getDetails(GOVERNANCE_SUBTYPE_TBL, '*', "status~~~url_key","ACTIVE~~~$url_key",'=~~~=~~~=', '', '' , "");
if(intval(count($rsDET)) == intval(0)){
       header("location: ".SITE_ROOT."not-found.php");
        exit();
}
$typeDESCRIPTION = "";
$subtypeDESCRIPTION = (stripslashes($rsDET[0]['subtype_description']));
$subtypeNAME = htmlentities((stripslashes($rsDET[0]['subtype_name'])));
$subtypeID = intval($rsDET[0]['subtype_id']);
...
<div class="container">
    <div class="clearfix inner_page">
        ...
        <div class="col-md-10 col-sm-9 inner_page_right pagination__list">
        	<h2><?php echo $subtypeNAME; ?> </h2>
            <?php 
                if($subtypeDESCRIPTION != ""){
            ?>
			<p><?php echo $subtypeDESCRIPTION; ?></p>
           ...
           <?php } ?>
            ...
            <div class="clearfix organisation">
                    <?php 
                    //$typdVAL = getDetails(GOVERNANCE_TBL, '*', "status~~~subtype_id","ACTIVE~~~$subtypeID",'=~~~=~~~=', 'governance_name', '' , ""); 
                    //$numGOV = count($typdVAL);
                   
                    $SQL1  = ""; 
                    $SQL1 .= " SELECT COUNT(*) AS CT FROM " . GOVERNANCE_TBL . " as G "; 
                    $SQL1 .= " WHERE status = 'ACTIVE' and subtype_id = :subtype_id  ";
                    $SQL1 .= " $search "; 
                    ...
                    $SQL .= " AND subtype_id =:subtype_id ";
                   // if(($url_key == "national-committee-for-regional-affairs") || ($url_key == "insolvency-and-bankruptcy-moot-competition-steering-committee")){
                   ...
                   $stmt1->bindParam(":subtype_id", $subtypeID); 
                    ...
                    $stmt2->bindParam(":subtype_id", $subtypeID); 
                    $stmt2->execute();
                        ...
                        <?php
                            ...
                            { 
                                ...
                                { 
                                    ...
                                    $masterURL = SITE_ROOT . urlRewrite(PAGE_MAIN, array("type_url_key"=> $type_url_key, "subtype_key"=>$url_key, "url_key" => $masterURLKEY));
