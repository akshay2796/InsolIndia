<?php include('header.php'); 
    define("PAGE_MAIN","young_members_committee_detail.php");
    define("PAGE_LIST","young-members-committee.php");
...
<div class="container">
    <div class="clearfix inner_page">
        ...
        <div class="col-md-10 col-sm-9 inner_page_right pagination__list">
        	<h2>Young Practitioner's Committee <span style="font-size: 18px;">(In Alphabetical Order)</span></h2>
            <div class="clearfix organisation">
                    <?php 
                  /*  $SQL1  = ""; 
                    $SQL1 .= " SELECT COUNT(*) AS CT FROM " . YOUNG_MEMBER_COMMITTEE_TBL . " as G "; 
                    $SQL1 .= " WHERE status = 'ACTIVE' and member_name !=''  ";
                    $SQL1 .= " $search "; */
                    ...
                    $SQL = "";
                    $SQL .= " SELECT * ";
                    $SQL .= " FROM ".YOUNG_MEMBER_COMMITTEE_TBL;
                    $SQL .= " WHERE status = 'ACTIVE' and member_name !='' ";
                    $SQL .= " $search ";
                    $SQL .= " order by member_name, position ";
                    //echo $SQL;
                        ...
                        <?php
                            ...
                            { 
                                foreach($rsLIST as $rLIST)
                                {
                                    ...
                                    $masterID = intval($rLIST['member_id']);
                                    $masterNAME = htmlentities(stripslashes($rLIST['member_name']));
                                    $masterPOST = htmlentities(stripslashes($rLIST['member_post']));
                                    $masterPROFESSION= htmlentities(stripslashes($rLIST['member_profession']));
                                    $alt_text = $masterNAME;
                                    ...
                                    $R200_IMG_EXIST = chkImageExists(MODULE_UPLOADIFY_ROOT . FLD_YOUNG_MEMBER_COMMITTEE. "/R200-" . $masterIMG);
                                    
                                    if( intval($R200_IMG_EXIST) == intval(1) )
                                    {
                                        $DISPLAY_IMG = MODULE_FILE_FOLDER . FLD_YOUNG_MEMBER_COMMITTEE. "/R200-" . $masterIMG;
                                    }
                                    else
                                    {
