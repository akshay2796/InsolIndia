...
if (intval($member_id) > intval(0))
{
	...
	if(intval(count($rsGET))>intval(0))
	{
        ...
        $member_since_date = "";
        if(intval($sig_company_id) > intval(0))
        { 
            $member_since_date = getDetails(SIG24_TBL, 'member_since_date', "sig24_id","$sig_company_id",'=', '', '' , "");
            if($member_since_date !='0000-00-00' && $member_since_date !='0000-00-00 00:00:00' && $member_since_date !="")
            {
                //$membership_start_date = date('d-m-Y', strtotime($member_since_date));
	...
	<div class="hrline"></div>
        ...
        <div class="fullDivider">
       	...
       	<div class="fullWidth" style=" padding-bottom:0px;">
               ...
               <div class="width3" id="showMEMDATE" <?php if($sig_member == intval(0)){ echo "style='display:none'"; }?> >
                	<label class="mainLabel">Membership Start Date <span>*</span> <?php if ($member_since_date !='') { ?>[<?php echo $member_since_date;?>] <?php } ?></label> 
                    <div class="txt3Box">
