...
function removeImage()
{
    ...
    else
    {
        if(unlink(CMS_UPLOAD_FOLDER_RELATIVE_PATH . $FOLDER_NAME . "/" . $image_name)) 
        {
            ...
            $SQL = "";
            $SQL .= "UPDATE " . EXECUTIVE_COMMITTEE_TBL . " SET " ;
            $SQL .= " image_name = :image_name, ";
            $SQL .= " image_id = :image_id ";
            $SQL .= " WHERE executive_id = :executive_id ";
            ...
            $stk_upd->bindParam(":executive_id", $imageId);
            $stk_upd->execute();
...
function saveData()
{
    ...
    $executive_type = trustme($_REQUEST['executive_type']);
    $executive_name = trustme($_REQUEST['executive_name']);
    $executive_post = trustme($_REQUEST['executive_post']);
    $executive_email = trustme($_REQUEST['executive_email']);
    $executive_profession = trustme($_REQUEST['executive_profession']);    
    
    $executive_profile = trustyou($_REQUEST['dcontent']);
    ...
    $url_key = filterString($executive_name);
    ...
    $meta_title = trim($meta_title) == '' ? trim($executive_name) : trim($meta_title);
    $meta_keyword = trim($meta_keyword) == '' ? trim($executive_name) : trim($meta_keyword);
    $meta_description = trim($meta_description) == '' ? trim($executive_name) : trim($meta_description);
    ...
    $FOLDER_NAME = "";
    $FOLDER_NAME = CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_EXECUTIVE_COMMITTEE . "/";
    
    if($con == "add")
    {
        $CHK = checkDuplicate(EXECUTIVE_COMMITTEE_TBL,"executive_name","$executive_name","=","");
        //echo $section_name;       
                 
        if( intval($CHK) == intval(0) )
        {
            
            $MAX_ID = getMaxId(EXECUTIVE_COMMITTEE_TBL,"executive_id");
            $MAX_POS = getMaxPosition(EXECUTIVE_COMMITTEE_TBL,"position","","","=");
            $MY_URLKEY = getURLKEY(EXECUTIVE_COMMITTEE_TBL,$url_key,$executive_name,"","","",$MAX_ID,"");
            
            if($image != "")
            {
                $title_filter = filterString($executive_name);
            ...
            $sql  = "";
            $sql .= " INSERT INTO " . EXECUTIVE_COMMITTEE_TBL . " SET ";
            $sql .= " executive_id = :executive_id, ";
            $sql .= " executive_type = :executive_type, ";
            $sql .= " executive_name = :executive_name, ";
            $sql .= " executive_email = :executive_email, ";
            $sql .= " executive_post = :executive_post, ";
            $sql .= " executive_profession = :executive_profession, ";            
            $sql .= " executive_profile = :executive_profile, ";
            $sql .= " image_name = :image_name, ";
            $sql .= " image_id = :image_id, ";
            ...
            $stmt->bindParam(":executive_id", $MAX_ID);
            $stmt->bindParam(":executive_type", $executive_type);
            $stmt->bindParam(":executive_name", $executive_name);
            $stmt->bindParam(":executive_email", $executive_email);
            $stmt->bindParam(":executive_post", $executive_post);
            $stmt->bindParam(":executive_profession", $executive_profession);            
            $stmt->bindParam(":executive_profile", $executive_profile);
    ...
    else if($con == "modify")
    {
        ...
        $CHK = checkDuplicate(EXECUTIVE_COMMITTEE_TBL,"executive_name~~~executive_id","$executive_name~~~$id","=~~~<>","");
        
        
        if( intval($CHK) == intval(0) )
        { 
            $MY_URLKEY = getURLKEY(EXECUTIVE_COMMITTEE_TBL,$url_key,$executive_name,"executive_id",$id,"<>",$id); 
            ...
            $stmt->bindParam(":url_key", $MY_URLKEY);
            $stmt->bindParam(":executive_id", $id); 
            $rs = $stmt->execute();  
            $stmt->closeCursor();            
...
function listData()
{
    ...
    $executive_type = trustme($_REQUEST['search_executive_type']);
    $executive_post = trustme($_REQUEST['search_executive_post']);
    $executive_name = trustme($_REQUEST['search_executive_name']);
    
    $search = "";
    
    if(trim($executive_type) != "")
    {
        $search .= " and executive_type = :executive_type ";
    }
    if(trim($executive_post) != "")
    {
        $search .= " and executive_post like :executive_post ";
    }
    if($executive_name != "")
    {
        $search .= " and executive_name like :executive_name ";
    }
    ...
    $SQL = "";
    $SQL .= " SELECT A.* ";     
    $SQL .= " FROM " . EXECUTIVE_COMMITTEE_TBL . " as A WHERE  A.status <> 'DELETE' $search ORDER BY position, executive_id desc "; 
    
    $SQL_PG = " SELECT COUNT(*) AS CT FROM " . EXECUTIVE_COMMITTEE_TBL . " WHERE status <> 'DELETE' $search ";
         
    $stmt1 = $dCON->prepare($SQL_PG);
    //echo $SQL;
    if(trim($executive_type)  != "")
    {
        $stmt1->bindParam(":executive_type", $executive_type);        
    }
    if(trim($executive_post)  != "")
    {
        $stmt1->bindParam(":executive_post", $executive_post_search);
        $executive_post_search = "%{$executive_post}%";
    }
    if($executive_name != "")
    {
        $stmt1->bindParam(":executive_name", $personnelname);
        $personnelname = "%{$executive_name}%";
    }
    ...
    if(trim($executive_type)  != "")
    {
        $stmt2->bindParam(":executive_type", $executive_type);        
    }
    
    if(trim($executive_post)  != "")
    {
        $stmt2->bindParam(":executive_post", $executive_post_search);
        $executive_post_search = "%{$executive_post}%";
    }
    
    if($executive_name != "")
    {
        $stmt2->bindParam(":executive_name", $personnelname);
        $personnelname = "%{$executive_name}%";
    }
    ...
    $position_qry_string = "";
    $position_qry_string .= "con=".EXECUTIVE_COMMITTEE_TBL;
    $position_qry_string .= "&cname1=executive_name&cname2=executive_id";
                ...
                <td class="list_table" valign="top">
                    ...
                    <table cellpadding="0" cellspacing="0" width="100%" border='0'>
                        <tr>
                            ...
                            <th width="20%" align="left">Profession</th>                            
                            ...
                            <td colspan="9" style="padding:0px; border-bottom:0px;">                               
                                <ul id="test-list" style="padding-left: 0px;list-style: none;">                      
                                    ...
                                    foreach($row as $rs)
                                    {
                                        ...
                                        $executive_id = intval($rs['executive_id']);
                                        $executive_type = stripslashes($rs['executive_type']);
                                        $executive_name = stripslashes($rs['executive_name']);
                                        $executive_post = stripslashes($rs['executive_post']);
                                        $executive_profession = stripslashes($rs['executive_profession']);                                        
                                        ...
                                        $DISPLAY_IMG = "";
                                        $R200_IMG_EXIST = chkImageExists(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_EXECUTIVE_COMMITTEE . "/R200-" . $image_name);
                                        //$MAIN_IMG_EXIST = chkImageExists(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_EXECUTIVE_COMMITTEE . "/" . $image_name);
                                        
                                        if( intval($R200_IMG_EXIST) == intval(1) )
                                        {
                                            $DISPLAY_IMG = CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_EXECUTIVE_COMMITTEE . "/R200-" . $image_name;
                                        }
                                        ?>     
                                        
                                        <li id="listItem_<?php echo $executive_id; ?>"> 
                                            <table cellpadding="0" cellspacing="0" width="100%" border="0">
                                                <tr class="expiredCoupons trhover" >
                                        
                                                    <td align="center" width="5%"> 
                                                        <input type="checkbox" class="cb-element" name="chk[]" value="<?php echo $executive_id; ?>" />
                                                    </td>
                                                    
                                                    <td align="center" width="5%">
                                                       <div id="INPROCESS_STATUS_1_<?php echo $executive_id; ?>" style="display: none;"></div>
                                                        <div id="INPROCESS_STATUS_2_<?php echo $executive_id; ?>"  >
                                                            <a href="javascript:void(0);" value="<?php  echo $executive_id; ?>" myvalue="<?php echo $STATUSnowaction; ?>" class="setStatus">
                                                                <img 
                                                                <?php 
                                                    ...
                                                    <td width="10%">
                                                    	<h3 class="couponType">
                                                            <?php echo ucwords(strtolower($executive_type)); ?>                                                            
                                                       </h3>
                                                    </td>
                                                    <td width="20%">
                                                    	<h3 class="couponType">
                                                            <?php echo ucwords(strtolower($executive_name)); ?>                                                            
                                                       </h3>
                                                    </td>
                                                    <td width="15%">
                                                	   <?php echo $executive_post; ?>
                                                    </td>   
                                                    <td width="20%">
                                                	   <?php echo $executive_profession; ?>
                                                    </td>                                      
                                                    ...
                                                    <td align="center" width="10%">
                                                       <div id="INPROCESS_DELETE_1_<?php echo $executive_id; ?>" style="display: none;"></div>
                                                       
                                                       <div id="INPROCESS_DELETE_2_<?php echo $executive_id; ?>">
                                                            <a href="<?php echo PAGE_MAIN; ?>?id=<?php echo $executive_id; ?>" class="modifyData img_btn">
                                                            ...
                                                            {
                                                            ?>
                                                                <a href="javascript:void(0);" value="<?php  echo $executive_id; ?>" class="deleteData img_btn">
...
function deleteSelected()
{
    ...
    foreach($exp as $chk)
    {
        ...
        ///$stmt = $dCON->prepare("DELETE FROM " . EXECUTIVE_COMMITTEE_TBL . "   WHERE section_id = ?");
        $stmt = $dCON->prepare("Update " . EXECUTIVE_COMMITTEE_TBL . "  set status='DELETE' WHERE executive_id = ?");
...
function setStatus()
{
    ...
    $STR  = "";
    $STR .= " UPDATE  " . EXECUTIVE_COMMITTEE_TBL . "  SET "; 
    $STR .= " status = :status, ";
    $STR .= " update_by = :update_by, ";
    $STR .= " update_ip = :update_ip, ";
    $STR .= " update_time = :update_time ";
    $STR .= " WHERE executive_id = :executive_id ";
    ...
    $sDEF->bindParam(":executive_id", $ID);
    $RES = $sDEF->execute();
    $sDEF->closeCursor();         
