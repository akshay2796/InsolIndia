...
function saveData()
{
    ...
    $member_type_id = trustme($_REQUEST['member_type_id']);
    ...
    $FOLDER_NAME = "";
    $FOLDER_NAME = CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_MEMBER_DIRECTORY . "/";
    
    if($con == "add")
    {
        $CHK = checkDuplicate(MEMBER_TBL,"member_type_id~~~member_name","$member_type_id~~~$member_name","=~~~=","");
        //echo $section_name;       
                 
        if( intval($CHK) == intval(0) )
        {
            
            $MAX_ID = getMaxId(MEMBER_TBL,"member_id");
            $MAX_POS = getMaxPosition(MEMBER_TBL,"position","","","=");
            $MY_URLKEY = getURLKEY(MEMBER_TBL,$url_key,$member_name,"","","",$MAX_ID,"");
            ...
            $sql  = "";
            $sql .= " INSERT INTO " . MEMBER_TBL . " SET ";
            $sql .= " member_id = :member_id, ";
            $sql .= " member_type_id = :member_type_id, ";
            ...
            $stmt->bindParam(":member_type_id", $member_type_id);
    ...
    else if($con == "modify")
    {
        ...
        $CHK = checkDuplicate(MEMBER_TBL,"member_type_id~~~member_name~~~member_id","$member_type_id~~~$member_name~~~$id","=~~~=~~~<>","");
        
        
        if( intval($CHK) == intval(0) )
        { 
            $MY_URLKEY = getURLKEY(MEMBER_TBL,$url_key,$member_name,"member_id",$id,"<>",$id); 
...
function listData()
{
    ...
    $search = "";
    if($member_type_id > intval(1))
    {
        $search .= " and member_type_id = '" . $member_type_id . "' ";        
    }   
    ...
    $SQL = "";
    $SQL .= " SELECT A.* ";     
    $SQL .= " FROM " . MEMBER_TBL . " as A WHERE  A.status <> 'DELETE' $search ORDER BY position, member_id desc "; 
    
    $SQL_PG = " SELECT COUNT(*) AS CT FROM " . MEMBER_TBL . " WHERE status <> 'DELETE' $search ";
    ...
    $position_qry_string = "";
    $position_qry_string .= "con=".MEMBER_TBL;
                ...
                <td class="list_table" valign="top">
                    ...
                    <table cellpadding="0" cellspacing="0" width="100%" border='0'>
                        <tr>
                            ...
                            <th width="45%" align="left">Profession</th>                            
                            ...
                            <td colspan="8" style="padding:0px; border-bottom:0px;">                               
                                <ul id="test-list" style="padding-left: 0px;list-style: none;">                      
                                    ...
                                    foreach($row as $rs)
                                    {
                                        ...
                                        $DISPLAY_IMG = "";
                                        $R200_IMG_EXIST = chkImageExists(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_MEMBER_DIRECTORY . "/R200-" . $image_name);
                                        //$MAIN_IMG_EXIST = chkImageExists(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_MEMBER_DIRECTORY . "/" . $image_name);
                                        
                                        if( intval($R200_IMG_EXIST) == intval(1) )
                                        {
                                            $DISPLAY_IMG = CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_MEMBER_DIRECTORY . "/R200-" . $image_name;
                                        }
                                        ?>     
                                                    ...
                                                    <td align="center" width="5%">
                                                       <div id="INPROCESS_STATUS_1_<?php echo $member_id; ?>" style="display: none;"></div>
                                                        <div id="INPROCESS_STATUS_2_<?php echo $member_id; ?>"  >
                                                            <a href="javascript:void(0);" value="<?php  echo $member_id; ?>" myvalue="<?php echo $STATUSnowaction; ?>" class="setStatus">
                                                                <img 
                                                                <?php 
                                                   ...
                                                   <!-- </td> -->
                                                    ...
                                                    <td align="center" width="10%">
                                                       ...
                                                       <div id="INPROCESS_DELETE_2_<?php echo $member_id; ?>">
                                                            <a href="<?php echo PAGE_MAIN; ?>?id=<?php echo $member_id; ?>" class="modifyData img_btn">
...
function deleteSelected()
{
    ...
    foreach($exp as $chk)
    {
        ...
        ///$stmt = $dCON->prepare("DELETE FROM " . MEMBER_TBL . "   WHERE section_id = ?");
        $stmt = $dCON->prepare("Update " . MEMBER_TBL . "  set status='DELETE' WHERE member_id = ?");
