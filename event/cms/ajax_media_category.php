...
define("PAGE_MAIN","media_category.php");	
define("PAGE_AJAX","ajax_media_category.php");
...
function saveData()
{
    ...
    $category_name = trustme($_REQUEST['category_name']);
    ...
    $url_key = filterString($category_name);
    ...
    $meta_title = trim($meta_title) == '' ? trim($category_name) : trim($meta_title);
    $meta_keyword = trim($meta_keyword) == '' ? trim($category_name) : trim($meta_keyword);
    $meta_description = trim($meta_description) == '' ? trim($category_name) : trim($meta_description);
       
    
    if($con == "add")
    {
        $CHK = checkDuplicate(MEDIA_CATEGORY_TBL,"category_name","$category_name","=","");
        //echo $category_name;       
                 
        if( intval($CHK) == intval(0) )
        {
            
            $MAX_ID = getMaxId(MEDIA_CATEGORY_TBL,"category_id");
            $MAX_POS = getMaxPosition(MEDIA_CATEGORY_TBL,"position","","","=");
              
            $MY_URLKEY = getURLKEY(MEDIA_CATEGORY_TBL,$url_key,$url_key,"","","",$MAX_ID);  
                             
            $sql  = "";
            $sql .= " INSERT INTO " . MEDIA_CATEGORY_TBL . " SET ";
            ...
            $stmt->bindParam(":category_id", $MAX_ID);
    ...
    {
        $CHK = checkDuplicate(MEDIA_CATEGORY_TBL,"category_name~~~category_id",$category_name."~~~".$id,"=~~~<>","");
         
        if( intval($CHK) == intval(0) )
        { 
            $MY_URLKEY = getURLKEY(MEDIA_CATEGORY_TBL,$url_key,$url_key,"category_id",$id,"<>",$id);  
            
            
            $sql  = "";
            $sql .= " UPDATE " . MEDIA_CATEGORY_TBL . " SET ";
            $sql .= " category_id = :category_id, ";
            $sql .= " category_name = :category_name, ";
            $sql .= " status = :status, ";
...
{ 
    ...
    $SQL = "";
    $SQL .= " SELECT * FROM " .  MEDIA_CATEGORY_TBL . " as B ";
    $SQL .= " WHERE B.status <> 'DELETE' ";
    $SQL .= " ORDER BY position,category_id "; 
    //echo $SQL; 
    ...
    $position_qry_string = "";
    $position_qry_string .= "con=".MEDIA_CATEGORY_TBL;
    $position_qry_string .= "&cname1=category_name&cname2=category_id";
    ...
    ?>
        ...
        <script language="javascript" type="text/javascript">
            $(document).ready(function(){
                ...
                $(".deleteData").click(function(){
                    ...
                    $(this).deleteData({category_id: category_id});  
                });
                ...
                $(".modifyData").click(function(){
                    ...
                    $(this).modifyData({category_id: category_id});
        ...
        <div class="containerPad" >
            ...
            foreach($row as $rs)
            {
                ...
                $category_id = stripslashes($rs['category_id']);
                $category_name = stripslashes($rs['category_name']); 
                
                $shipping_partner_url = stripslashes($rs['shipping_partner_url']); 
                $status_type_default = stripslashes($rs['status_type_default']); 
                $DEFnowaction = trim($status_type_default) == 'YES' ? "NO" : "YES";  
                ...
                $CHK = chkFor($category_id);   
                 
                
                ?>
                
                <div class="smallListBox" id="listItem_<?php echo $category_id; ?>">
               		<div class="sListTitle"><?php echo $category_name; ?></div>                        
                        	...
                        	<tr>
                            	<td align="left" width="20">
                                    ...
                                    if( intval($CHK) == intval(0) )
                                    {
                                        $CK_COUNTER++;
                                    ?>
                                        <input type="checkbox" class="cb-element" name="chk[]" value="<?php echo $category_id; ?>" />
                                    <?php
                                    }
                                    else
                                    {
                                ...
                                <td width="30" class="statusTd">
                                	<div id="INPROCESS_STATUS_1_<?php echo $category_id; ?>" style="display: none;"></div>
                                    <div id="INPROCESS_STATUS_2_<?php echo $category_id; ?>"  >
                                        <?php 
                                        if( trim($status) == 'ACTIVE') 
                                        { 
                                        ?>
                                            <a href="javascript:void(0);" value="<?php  echo $category_id; ?>" myvalue="<?php echo $STATUSnowaction; ?>" class="setStatus"><img src="<?php echo CMS_INCLUDES_ICON_RELATIVE_PATH; ?>active.png" /></a>
                                        <?php 
                                        }
                                        elseif( trim($status) == 'INACTIVE') 
                                        { 
                                        ?>
                                           <a href="javascript:void(0);" value="<?php  echo $category_id; ?>" myvalue="<?php echo $STATUSnowaction; ?>" class="setStatus"><img src="<?php echo CMS_INCLUDES_ICON_RELATIVE_PATH; ?>inactive.png" /></a>
                                        <?php 
                                        }
                                        ?>   
                                    </div>
                                 ...
                                 <td align="right">                                
                                    ...
                                    <div id="INPROCESS_DELETE_1_<?php echo $category_id; ?>" style="display: none;"></div>
                                    <div id="INPROCESS_DELETE_2_<?php echo $category_id; ?>"  >
                                        <a href="javascript:void(0);" value="<?php echo $category_id; ?>" class="modifyData cmsIcon" title="Modify">
                                        <img src="<?php echo CMS_INCLUDES_ICON_RELATIVE_PATH; ?>edit_icon.png" border="0" title="Modify" alt="Modify" /></a> 
                                        <?php
                                        if( intval($CHK) == intval(0) )
                                        {
                                        ?>
                                            <a href="javascript:void(0);" value="<?php  echo $category_id; ?>" class="deleteData cmsIcon">
...
function modifyData()
{
    ...
    $stmt = $dCON->prepare(" SELECT * FROM " . MEDIA_CATEGORY_TBL . " WHERE category_id = ? ");
    $stmt->bindParam(1, $category_id);
    $stmt->execute();
    $row_stmt = $stmt->fetchAll();
    $stmt->closeCursor();
    
    $category_id = intval($row_stmt[0]['category_id']);
    $category_name = (stripslashes($row_stmt[0]['category_name'])); 
    $meta_title = (stripslashes($row_stmt[0]['meta_title'])); 
    $meta_keyword = (stripslashes($row_stmt[0]['meta_keyword'])); 
    $meta_description = (stripslashes($row_stmt[0]['meta_description'])); 
    $status = stripslashes($row_stmt[0]['status']);
    
     
    echo "~~~$category_id~~~$category_name~~~$meta_title~~~$meta_keyword~~~$meta_description~~~$status~~~";
...
function deleteSelected()
{
    ...
    foreach($exp as $chk)
    {
        ...
        $stmt = $dCON->prepare("DELETE FROM " . MEDIA_CATEGORY_TBL . "   WHERE category_id = ?");
...
function setStatus()
{
    ...
    $STR  = "";
    $STR .= " UPDATE  " . MEDIA_CATEGORY_TBL . "  SET "; 
    $STR .= " status = :status, ";
...
function chkFor($ID)
{
    ...
    $SQL = " SELECT SUM(CT) FROM ( ";
        $SQL .= " SELECT COUNT(*) AS CT FROM " . MEDIA_TBL . " WHERE category_id = ? AND status <> 'DELETE' ";
        //$SQL .= " UNION ";
