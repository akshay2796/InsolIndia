...
$CID = intval(base64_decode($_REQUEST['CID']));

$QRY = "";
$QRY = " select * from ".MEDIA_CATEGORY_TBL." AS s where s.status= 'ACTIVE' and category_id=:category_id";
//echo "==".$QRY.$CID;
//exit();
$sEXT = $dCON->prepare($QRY);
$sEXT->bindParam(":category_id",$CID);
$sEXT->execute();
$rsEXT = $sEXT->fetchAll();
$sEXT->closeCursor();
/*if(intval(count($rsEXT)) <= intval(0)){
    header("Location:media.php");    
}else{
    $category_name = htmlentities(stripslashes($rsEXT[0]['category_name']));
}*/
...
$(document).ready(function(){
     ...
     $.fn.listData = function() {
        ...
        $.ajax({
            ...
            data: "type=listData&CID=<?php echo $CID;?>&" +search_val + "&page=" + pageNo,
            beforeSend: function(){  
...
<h1>Media List <!--<span> - <?php echo $category_name;?></span>--> <div class="addProductBox"><a href="<?php echo PAGE_MAIN;?>?CID=<?php echo base64_encode(intval($CID));?>" class="backBtn">Add New</a></div></h1>
        ...
        <form id="frm" name="frm" method="post" action="" enctype="multipart/form-data">
            <div class="fullWidth noGap">
                <div class="width4">
                    ...
                    <input type="text" class="txtBox" name="search_name" id="search_name" value="" AUTOCOMPLETE="OFF"/>
                </div>
