...
define("PAGE_LIST","board_governors_list.php");

if( !is_dir(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_BOARD_GOVERNORS))
{
    $mask=umask(0);
    mkdir(CMS_UPLOAD_FOLDER_RELATIVE_PATH .  FLD_BOARD_GOVERNORS, 0777); 
    umask($mask);      
}


$governor_id = intval($_REQUEST['id']);
if( intval($governor_id) > intval(0) )
{   
    $con = "modify"; 
    $id = intval($governor_id);
    
    $stmt = $dCON->prepare(" SELECT * FROM " . BOARD_GOVERNORS_TBL . " WHERE governor_id = ? ");
    $stmt->bindParam(1, $governor_id);
    $stmt->execute();
    $row_stmt = $stmt->fetchAll();
    $stmt->closeCursor();
    
    $governor_id = intval($row_stmt[0]['governor_id']);
    $governor_name = htmlentities(stripslashes($row_stmt[0]['governor_name'])); 
    $governor_email = htmlentities(stripslashes($row_stmt[0]['governor_email']));
    $governor_post = htmlentities(stripslashes($row_stmt[0]['governor_post']));
    $governor_profession = htmlentities(stripslashes($row_stmt[0]['governor_profession']));
    $governor_profile = stripslashes($row_stmt[0]['governor_profile']); 
...
$(document).ready(function(){
    ...
    $.validator.addMethod('chkDataText', function (data){
        if( $.trim(CKEDITOR.instances.governor_profile.getData()) == "" )
        {
            return 0;
        }
        else
    ...
    $("#frm").validate({
        ...
        submitHandler: function() {
            ...
            var dcontent = escape(CKEDITOR.instances.governor_profile.getData()); 
...
<h1>Board of Governor <div class="addProductBox"><a href="<?php echo PAGE_LIST;?>" class="backBtn">Go To List</a></div></h1>
    ...
    <div class="containerPad expendableBox1">
        <div class="fullWidth">                    	
            <div class="width4">
                ...
                <input type="text" class="txtBox" value="<?php echo $governor_name;?>" name="governor_name" id="governor_name" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
            <div class="width4">
                <label class="mainLabel">Post </label>
                <input type="text" class="txtBox" value="<?php echo $governor_post;?>" name="governor_post" id="governor_post" AUTOCOMPLETE = "OFF" maxlength="200"/>
            </div>
            <div class="width2">
                <label class="mainLabel">Profession <span></span><span></span></label>
                <input type="text" class="txtBox" value="<?php echo $governor_profession;?>" name="governor_profession" id="governor_profession" AUTOCOMPLETE = "OFF" maxlength="300"/>
            </div>            
        </div>
        <div class="fullWidth">
            <div class="width4">
                <label class="mainLabel">Email </label>
                <input type="text" class="txtBox" value="<?php echo $governor_email;?>" name="governor_email" id="governor_email" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
        </div>
        ...
        <div class="fullWidth div_content validateMsg">
            <textarea name="governor_profile" id="governor_profile" style="width:90%; height:230px; float:left;"><?php echo $governor_profile; ?></textarea>
            <script>
            CKEDITOR.replace("governor_profile",
                {
                    enterMode: 2,
