...
function saveData()
{
    ...
    if ($con == "add") {
        $CHK = checkDuplicate(COMMITTEES_TYPE_TBL, "type_name", "$type_name", "=", "");
        //echo $type_name;

        if (intval($CHK) == intval(0)) {

            $MAX_ID = getMaxId(COMMITTEES_TYPE_TBL, "type_id");
            $MAX_POS = getMaxPosition(COMMITTEES_TYPE_TBL, "position", "", "", "=");

            $MY_URLKEY = getURLKEY(COMMITTEES_TYPE_TBL, $url_key, $url_key, "", "", "", $MAX_ID, "");

            $sql = "";
            $sql .= " INSERT INTO " . COMMITTEES_TYPE_TBL . " SET ";
            $sql .= " type_id = :type_id, ";
        ...
        $CHK = checkDuplicate(COMMITTEES_TYPE_TBL, "type_name~~~type_id", $type_name . "~~~" . $id, "=~~~<>", "");

        if (intval($CHK) == intval(0)) {
            $MY_URLKEY = getURLKEY(COMMITTEES_TYPE_TBL, $url_key, $url_key, "type_id", $id, "<>", $id, "");

            $sql = "";
            $sql .= " UPDATE " . COMMITTEES_TYPE_TBL . " SET ";
            $sql .= " type_id = :type_id, ";
            $sql .= " type_name = :type_name, ";
...
function listData()
{
    ...
    $SQL = "";
    $SQL .= " SELECT * FROM " . COMMITTEES_TYPE_TBL . " as B ";
    ...
    $position_qry_string = "";
    $position_qry_string .= "con=" . COMMITTEES_TYPE_TBL;
...
function modifyData()
{
    ...
    $stmt = $dCON->prepare(" SELECT * FROM " . COMMITTEES_TYPE_TBL . " WHERE type_id = ? ");
...
function deleteSelected()
{
    ...
    foreach ($exp as $chk) {
        ...
        $stmt = $dCON->prepare("DELETE FROM " . COMMITTEES_TYPE_TBL . "   WHERE type_id = ?");
...
function setStatus()
{
    ...
    $STR = "";
    $STR .= " UPDATE  " . COMMITTEES_TYPE_TBL . "  SET ";
    $STR .= " status = :status, ";
...
function chkFor($ID)
{
    ...
    $SQL .= " SELECT COUNT(*) AS CT FROM " . COMMITTEES_TBL . " WHERE committee_type_id = ? AND status <> 'DELETE' ";
    //$SQL .= " UNION ";
