...
define("PAGE_MAIN","resources_category.php");	
define("PAGE_AJAX","ajax_resources_category.php");
...
function saveData()
{
    ...
    if($con == "add")
    {
        $CHK = checkDuplicate(RESOURCES_CATEGORY_TBL,"category_name","$category_name","=","");
        //echo $category_name;       
                 
        if( intval($CHK) == intval(0) )
        {
            
            $MAX_ID = getMaxId(RESOURCES_CATEGORY_TBL,"category_id");
            $MAX_POS = getMaxPosition(RESOURCES_CATEGORY_TBL,"position","","","=");
              
            $MY_URLKEY = getURLKEY(RESOURCES_CATEGORY_TBL,$url_key,$url_key,"","","",$MAX_ID);  
                             
            $sql  = "";
            $sql .= " INSERT INTO " . RESOURCES_CATEGORY_TBL . " SET ";
    ...
    {
        $CHK = checkDuplicate(RESOURCES_CATEGORY_TBL,"category_name~~~category_id",$category_name."~~~".$id,"=~~~<>","");
         
        if( intval($CHK) == intval(0) )
        { 
            $MY_URLKEY = getURLKEY(RESOURCES_CATEGORY_TBL,$url_key,$url_key,"category_id",$id,"<>",$id);  
            
            
            $sql  = "";
            $sql .= " UPDATE " . RESOURCES_CATEGORY_TBL . " SET ";
            $sql .= " category_id = :category_id, ";
            $sql .= " category_name = :category_name, ";
...
{ 
    ...
    $SQL = "";
    $SQL .= " SELECT * FROM " .  RESOURCES_CATEGORY_TBL . " as B ";
    ...
    $position_qry_string = "";
    $position_qry_string .= "con=".RESOURCES_CATEGORY_TBL;
...
function modifyData()
{
    ...
    $stmt = $dCON->prepare(" SELECT * FROM " . RESOURCES_CATEGORY_TBL . " WHERE category_id = ? ");
...
function deleteSelected()
{
    ...
    foreach($exp as $chk)
    {
        ...
        $stmt = $dCON->prepare("DELETE FROM " . RESOURCES_CATEGORY_TBL . "   WHERE category_id = ?");
...
function deleteData()
{
    ...
    $TIME = date("Y-m-d H:i:s");
    //$category_name = getDetails(AIRPORT_CITY_TBL, 'category_name', "category_id",$ID,'=', '', '' , "" );
...
function setStatus()
{
    ...
    $STR  = "";
    $STR .= " UPDATE  " . RESOURCES_CATEGORY_TBL . "  SET "; 
    $STR .= " status = :status, ";
...
function chkFor($ID)
{
    ...
    $SQL = " SELECT SUM(CT) FROM ( ";
        $SQL .= " SELECT COUNT(*) AS CT FROM " . RESOURCES_TBL . " WHERE category_id = ? AND status <> 'DELETE' ";
        //$SQL .= " UNION ";
