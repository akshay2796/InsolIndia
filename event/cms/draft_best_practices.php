...
define("PAGE_LIST","draft_best_practices_list.php");

if( !is_dir(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_DRAFT_BEST_PRACTICES))
{
    $mask=umask(0);
    mkdir(CMS_UPLOAD_FOLDER_RELATIVE_PATH .  FLD_DRAFT_BEST_PRACTICES, 0777); 
    umask($mask);      
}
...
define("FTYPE_FOLDER",FLD_DRAFT_BEST_PRACTICES . "/" . FLD_DRAFT_BEST_PRACTICES_FILE);
define("FTYPE_FOLDER_PATH",CMS_UPLOAD_FOLDER_RELATIVE_PATH . FTYPE_FOLDER); 

$draft_id = intval($_REQUEST['id']);
if( intval($draft_id) > intval(0) )
{   
    $con = "modify"; 
    $id = intval($draft_id);
    
    $stmt = $dCON->prepare(" SELECT * FROM " . DRAFT_BEST_PRACTICES_TBL . " WHERE draft_id = ? ");
    $stmt->bindParam(1, $draft_id);
    $stmt->execute();
    $row_stmt = $stmt->fetchAll();
    $stmt->closeCursor();
    
    $draft_id = intval($row_stmt[0]['draft_id']);
    $title_name = htmlentities(stripslashes($row_stmt[0]['title_name'])); 
   // $url = htmlentities(stripslashes($row_stmt[0]['url']));
    ...
    $file_name = stripslashes($row_stmt[0]['file_name']);
    $FTYPE_PATH = CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_DRAFT_BEST_PRACTICES . "/" . FLD_DRAFT_BEST_PRACTICES_FILE."/";    
    $chKFTYPE = chkImageExists($FTYPE_PATH .$file_name);    
    $old_ftype_file = stripslashes($row_stmt[0]['file_name']);
...
<h1>Draft Insolvency Best Practices <div class="addProductBox"><a href="<?php echo PAGE_LIST;?>" class="backBtn">Go To List</a></div></h1>
    ...
    <div class="containerPad expendableBox1">
        ...
        <div class="fullWidth">                    	
            <div class="width">
                ...
                <input type="text" class="txtBox" value="<?php echo $title_name;?>" name="title_name" id="title_name" AUTOCOMPLETE = "OFF" maxlength="500"/>
            </div>
        </div>
		<div class="clear">&nbsp;</div>
        <div class="fullDivider noGap">
