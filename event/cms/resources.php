...
define("VIDEO_FOLDER",FLD_RESOURCES . "/" . FLD_RESOURCES_VDO);
...
define("FTYPE_FOLDER",FLD_RESOURCES . "/" . FLD_RESOURCES_FILE);
...
if( (intval($ID) > intval(0)) )
{
    ...
    $SQL  = "";
    $SQL .= " SELECT * FROM " . RESOURCES_TBL . " as A ";
    $SQL .= " WHERE status <> 'DELETE' ";
    $SQL .= " AND resources_id = :resources_id ";
    //echo "<BR>" . $SQL . "<BR>".$ID;
    //exit();
    $sGET = $dCON->prepare( $SQL );
    $sGET->bindParam(":resources_id", $ID);
    $sGET->execute();
    $rsGET = $sGET->fetchAll();
    $sGET->closeCursor();
    ...
    $resources_name = htmlentities(stripslashes($rsGET[0]['resources_name']));  
    $resources_publisher = htmlentities(stripslashes($rsGET[0]['resources_publisher'])); 
    $publication_month = stripslashes($rsGET[0]['publication_month']); 
    $publication_year = stripslashes($rsGET[0]['publication_year']);
    $resources_from_date = stripslashes($rsGET[0]['resources_from_date']);
    
    if(trim($resources_from_date) != "" && $resources_from_date != "0000-00-00"){
        $resources_from_date = date('d-m-Y' , strtotime($resources_from_date));    
    }else{
        $resources_from_date = "";
    }
    
    $resources_type = stripslashes($rsGET[0]['resources_type']);
    $resources_url = stripslashes($rsGET[0]['resources_url']);
    $file_name = stripslashes($rsGET[0]['file_name']);
    $FTYPE_PATH = CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_RESOURCES . "/" . FLD_RESOURCES_FILE . "/";    
    $chKFTYPE = chkImageExists($FTYPE_PATH .$file_name); 
 
    
    
    $resources_short_description = htmlentities(stripslashes($rsGET[0]['resources_short_description']));  
    $resources_description = stripslashes($rsGET[0]['resources_description']);  
    ...
    $VTYPE_PATH = CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_RESOURCES . "/" . FLD_RESOURCES_VDO . "/";    
    ...
    if ( SET1_ENABLE == true ){
        ...
        $QRY = "";
        $QRY .= "SELECT * FROM " . RESOURCES_IMAGES_TBL . " WHERE master_id = :master_id order by default_image desc, position,image_id ";
        //echo $QRY . $ID . "<BR>";
...
$(function() {
    ...
    $(".setDefault").click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo PAGE_AJAX; ?>",
            data: "type=setDefaultImage&image_id="+image_id+"&foldername="+foldername+"&resources_id=<?php echo $ID; ?>",
            beforeSend: function(){
...
$(document).ready(function(){
    ...
    $.validator.addMethod('chkDataText', function (data){
        if( $.trim($("input[name=resources_type]:checked").val()) == "CONTENT" && $.trim(CKEDITOR.instances.resources_description.getData()) == "" )
        {
            return 0;
        }
        else
    ...
    $.validator.addMethod('chkDataUrl', function (data){
        if( $.trim($("input[name=resources_type]:checked").val()) == "URL" && $.trim($("#ftype_url").val()) == "" )
        {
            return 0;
        }
        else
    ...
    $.validator.addMethod('chkDataFile', function (data){
        if( $.trim($("input[name=resources_type]:checked").val()) == "FILE" && $.trim($("#ftype_file_validation").val()) == "" )
        {
            return 0;
        }
        else
    ...
    $("#frm").validate({
        ...
        messages: {
           ...
           // resources_from_date: ""
            ...
            ,ftype_url: ""
            ,resources_description: " * Content cannot be blank"
        ...
        submitHandler: function() {
            ...
            var resources_from_date = $('#resources_from_date').val();
            var publication_year = $('#publication_year').val();
            
            if(resources_from_date == "" && publication_year == "")
            {
                alert("Please choose At least One Between Published On OR Publication Year !")
                return false;
            }
            var dcontent = escape(CKEDITOR.instances.resources_description.getData()); 
    ...
    */
    ...
    $(".resources_type").click(function(){
       
        ///var resources_type = $(this).val();
        var resources_type = $(".resources_type:checked").val()
        
        ///alert(resources_type);
        if(resources_type == "URL")
        {            
            $(".div_content").hide();
            $(".div_file").hide();
            $(".div_url").show();
        ...
        else if(resources_type == "CONTENT")
        {           
            $(".div_url").hide();
            $(".div_file").hide();
            $(".div_content").show();
    ...
    $("#resources_publisher").autocomplete({
...
<form id="frm" name="frm" method="post" action="" enctype="multipart/form-data">
    ...
    <input type="hidden" name="FTYPE_UPLOAD_IN_PROCESS" id="FTYPE_UPLOAD_IN_PROCESS" value="0" readonly style="display: none;"/>
...
<div class="addWrapper">
    ...
    <div class="containerPad">
        <div class="fullWidth">
            <div class="width4">
                <label class="mainLabel">Category <span>*</span></label>
                <select name="category_id" id="category_id" class="selectBox">
                    <option value="">--Select--</option>
                    <?php           
                    $rowCT = getDetails(RESOURCES_CATEGORY_TBL, '*', "status","ACTIVE",'=', 'category_name', '' , "");   
                    if ( count($rowCT) > intval(0) )
                    {                         
                        foreach($rowCT as $rsCT)
                        {
                    ?>
                            <option value="<?php echo intval($rsCT['category_id']); ?>" <?php if( intval($rsCT['category_id']) == intval($category_id) ) { echo " selected "; } ?> ><?php echo trim(stripslashes($rsCT['category_name']));?></option>
                    <?php
                        }
                    }
                    ?> 
            ...
            <div class="width2">
                ...
                <input type="text" class="txtBox" name="resources_name" id="resources_name" value="<?php echo $resources_name; ?>" maxlength="500" autocomplete="OFF" />
            </div>              
        </div>
        <div class="fullWidth noGap">
            <div class="width4">
            	<label class="mainLabel">Published On </label>
                <input type="text" class="titleTxt txtBox" name="resources_from_date" id="resources_from_date" value="<?php echo $resources_from_date; ?>" maxlength="12" placeholder="Date" autocomplete="OFF" />&nbsp;&nbsp;
            </div>   
            <div class="width4">
            	<label class="mainLabel">Publication Month </label>
                <select name="publication_month" id="publication_month" class="selectBox">
                    <option value="">--Select--</option>
                    <option value="January" <?php if($publication_month == 'January') { echo "selected=''"; }?>>January</option>
                    <option value="February" <?php if($publication_month == 'February') { echo "selected=''"; }?> >February</option>
                    <option value="March" <?php if($publication_month == 'March') { echo "selected=''"; }?> >March</option>
                    <option value="April" <?php if($publication_month == 'April') { echo "selected=''"; }?> >April</option>
                    <option value="May" <?php if($publication_month == 'May') { echo "selected=''"; }?> >May</option>
                    <option value="June" <?php if($publication_month == 'June') { echo "selected=''"; }?> >June</option>
                    <option value="July" <?php if($publication_month == 'July') { echo "selected=''"; }?> >July</option>
                    <option value="August" <?php if($publication_month == 'August') { echo "selected=''"; }?> >August</option>
                    <option value="September" <?php if($publication_month == 'September') { echo "selected=''"; }?> >September</option>
                    <option value="October" <?php if($publication_month == 'October') { echo "selected=''"; }?> >October</option>
                    <option value="November" <?php if($publication_month == 'November') { echo "selected=''"; }?> >November</option>
                    <option value="December"<?php if($publication_month == 'December') { echo "selected=''"; }?> >December</option>
                </select>
            </div> 
            <div class="width4">
            	<label class="mainLabel">Publication Year </label>
                <input type="text" class="titleTxt txtBox" name="publication_year" id="publication_year" value="<?php echo $publication_year; ?>" maxlength="4" placeholder="Year" autocomplete="OFF" onkeyup="checkNum(this)" />&nbsp;&nbsp;
            </div> 
            <div class="width4">
            	<label class="mainLabel">Publisher <span></span> </label>
                <input type="text" class="titleTxt txtBox" name="resources_publisher" id="resources_publisher" value="<?php echo $resources_publisher; ?>" maxlength="50" placeholder="Publisher"  />&nbsp;&nbsp;
        ...
        <div class="fullWidth validateMsg">        	
            ...
            <textarea class="txtBox" name="resources_short_description" id="resources_short_description" data-maxsize="" data-output="status1" wrap="virtual" style="height:100px;"><?php echo $resources_short_description; ?></textarea>   
            <br><span id="status1" style="font-size:11px"></span> <span style="font-size:11px">characters </span>    
        </div>
        
        <div class="fullDivider">
        	<div class="sml_heading">Resources Type <span></span> </div>
        </div>
        
        <div class="fullWidth">
            
            <div class="">
                <label class="radioGroup"><input type="radio" name="resources_type" class="resources_type" value="CONTENT" <?php if( trim($resources_type) == 'CONTENT') { echo " checked "; } ?> checked="checked"/> Content</label>       
                <label class="radioGroup"><input type="radio" name="resources_type" class="resources_type" value="URL" <?php if( trim($resources_type) == 'URL') { echo " checked "; } ?> /> Url</label>
                <label class="radioGroup"><input type="radio" name="resources_type" class="resources_type" value="FILE" <?php if( trim($resources_type) == 'FILE') { echo " checked "; } ?> /> File</label>
            </div>
        </div>
         
        
        <div class="fullWidth div_file validateMsg" <?php if( trim($resources_type) != 'FILE') { echo " style='display: none;' "; } ?>>
        ...
        <div class="fullWidth div_url" <?php if( trim($resources_type) != 'URL') { echo " style='display: none;' "; } ?>>
            <label class="mainLabel">Url <span>*</span></label>
            <input type="text" class="titleTxt txtBox" name="ftype_url" id="ftype_url" value="<?php echo $resources_url; ?>" maxlength="1000">
            <small>ie. http://google.com</small>
        </div>

         
        <div class="div_content" <?php if( trim($resources_type) != 'CONTENT') { echo " style='display: none;' "; } ?>>
            ...
            <div class="fullWidth validateMsg">
                <textarea name="resources_description" id="resources_description" style="width:90%; height:230px; float:left;"><?php echo $resources_description; ?></textarea>
                <script>
                CKEDITOR.replace("resources_description",
                    {
                        enterMode: 2,
            ...
            <div class="fullWidth noGap" id="divEMBED" <?php if($videoTYPE == "VIDEO_FILE") { echo " style='display:none;' "; } ?>>                    	
                <div class="width2">
                    ...
                    <textarea name="embed_code" id="embed_code" class="embedVideo" style="width: 100%; min-height: 50px;"><?php echo $videoEMBEDCODE; ?></textarea>
                </div>
            </div>
                ...
                <div class="fullWidth noGap" >
                    <div class="width3">
                        <div id="video_upload_container" >
                            ...
                            <div id="videofilelist" style="width: 100%; min-height: 30px;height: auto; border: 1px solid #b6b6b6; padding: 5px;float:left;" >
        ...
        <div class="fullWidth noGap" id="image_list" <?php if (trim($resources_type) == 'URL' ) { echo " style='display: none;' "; } ?> >
                <?php if ( SET1_ENABLE == true ){ ?>
                    <div class="fullWidth">
                        <div class="sml_heading" id="uploadImgPos">
                            <?php 
                            echo "Upload Resources " .  ucwords(strtolower(SET1_TYPE));
                            if ( SET1_IMAGE_MULTIPLE == true ){
                                 echo "s";    
                            }                            
