...
define("PAGE_MAIN","newsletter_president.php");	
define("PAGE_AJAX","ajax_newsletter_master.php");


if( !is_dir(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_NEWSLETTER_PRESIDENT))
{
    $mask=umask(0);
    mkdir(CMS_UPLOAD_FOLDER_RELATIVE_PATH .  FLD_NEWSLETTER_PRESIDENT, 0777);
    umask($mask);
}
...
if(count($rsGET)> intval(0))
{
    ...
    $FOLDER_NAME = FLD_NEWSLETTER_PRESIDENT; 
    $FOLDER_PATH = CMS_UPLOAD_FOLDER_RELATIVE_PATH. $FOLDER_NAME;  
    $con = "modify"; 
    $ID = intval($rsGET[0]['president_id']);
    $president_name = htmlentities(stripslashes($rsGET[0]['president_name']));  
 ...
 $(function() {
        ...
        else
        {
	  	...
	  	image_list_html = image_list_html + '<div class="removeImageTR uploadImgContainer">';
            	image_list_html = image_list_html + '<div class="uploadImgBox"><img src="<?php echo CMS_INCLUDES_IMAGES_RELATIVE_PATH; ?>blackUploadArea.png" class="fullImg"/>';
				    image_list_html = image_list_html + '<input type="hidden" class="president_id" name="president_id" value="0" /><input type="hidden" class="cl_r_image" id="image" name="image" value="' + file.name + '" />';
    ...
    $.fn.removeImage = function() {
        ...
        $.ajax({
            ...
            data: "type=removeImagePresident&image_name=" + cl_r_image + "&imageId=" + imageId + "&foldername="+foldername,
            beforeSend: function() {
...
$(document).ready(function(){
     ...
     $.validator.addMethod('chkDataText', function (data){
        if( $.trim(CKEDITOR.instances.president_text.getData()) == "" )
        {
            return 0;
        }
        else
    ...
    $("#frm").validate({
        ...
        messages: {
          president_name: ""  
          ,president_text: " * Message cannot be blank"
        },
        submitHandler: function() {
            ...
            */
            ...
            $.ajax({
               type: "POST",
               url: "<?php echo PAGE_AJAX; ?>",
               data: "type=savePresident&pcontent=" + pcontent + "&" +  value,
			   beforeSend: function(){
...
<div class="responsiveBox"> 
    ...
    <h1>Newsletter - President Message</h1>
        ...
        <div class="containerPad">
            <!--div class="fullWidth noGap" id="image_list">
                ...
                if(count($rsGET) > intval(0))
                {
                    ...
                    $president_image = trim(stripslashes($rsGET[0]['president_image']));
                    $president_id = trim(stripslashes($rsGET[0]['president_id']));
                    $chk_file = chkImageExists($FOLDER_PATH."/".$president_image);
                    if(intval($chk_file) == intval(1))
                    {
                        $display_image = $FOLDER_PATH."/".$president_image;
                    }
                    else
                    {
                        $display_image ="";
                    ...
                    {
            ?>
                        <div class="removeImageTR uploadImgContainer" id="listItem_<?php echo $president_id; ?>">
                            <div class="uploadImgBox"><img src="<?php echo CMS_INCLUDES_IMAGES_RELATIVE_PATH; ?>blackUploadArea.png" class="fullImg"/>
                                ...
                                <input type="hidden" class="cl_r_image" id="image" name="image" value="<?php echo $president_image;?>" />
                            	<div class="addPhotoIconTbl"><span><a href="javascript:void(0)"><img src="<?php echo $display_image; ?>" class="fixedImg imgno<?php echo $president_id;?>" /></a></span></div>       
                            </div>
                            <div class="uploadImgBtn">
                            	<table>
                                    <tr>
                                    	<td><a href="javascript:void(0);" onclick="$(this).removeImage({imageId:<?php echo $president_id;?>,foldername:'<?php echo $FOLDER_NAME; ?>'});" class="removeImage"><img src="<?php echo CMS_INCLUDES_IMAGES_RELATIVE_PATH; ?>cms-icon/trash.png" title="Delete Thumbnail" border="0" /></a><img src="<?php echo CMS_INCLUDES_IMAGES_RELATIVE_PATH; ?>loader-small.gif" class="removeImageLoader" style="display:none;" /> </td>
                                	</tr>
                                </table>
                            </div>
                        </div>      
