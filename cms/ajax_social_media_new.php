...
define("PAGE_MAIN", "social_media_new.php");
define("PAGE_AJAX", "ajax_social_media_new.php");
...
function removeImage()
{
    ...
    if ($imageId == intval(0)) {
        ...
        if (unlink(CMS_UPLOAD_FOLDER_RELATIVE_PATH . $FOLDER_NAME . "/" . $image_name)) {
            ...
            $stk_upd->bindParam(":socialmedia_id", $imageId);
            $stk_upd->execute();
...
function saveData()
{
    ...
    $meta_title = trim($meta_title) == '' ? trim($socialmedia_name) : trim($meta_title);
    $meta_keyword = trim($meta_keyword) == '' ? trim($socialmedia_name) : trim($meta_keyword);
    $meta_description = trim($meta_description) == '' ? trim($socialmedia_name) : trim($meta_description);
    ...
    if ($con == "add") {
        ...
        if (intval($CHK) == intval(0)) {
            ...
            $MY_URLKEY = getURLKEY(SOCIALMEDIA_TBL, $url_key, $socialmedia_name, "", "", "", $MAX_ID, "");

            if ($image != "") {
                $title_filter = filterString($socialmedia_name);
        ...
        if (intval($CHK) == intval(0)) {
            $MY_URLKEY = getURLKEY(SOCIALMEDIA_TBL, $url_key, $socialmedia_name, "socialmedia_id", $id, "<>", $id, "");
...
function listData()
{
    ...
    $SQL = "";
    $SQL .= " SELECT A.* ";
    $SQL .= " FROM " . SOCIALMEDIA_TBL . " as A WHERE  A.status <> 'DELETE' $search ORDER BY news_date DESC ";

    $SQL_PG = " SELECT COUNT(*) AS CT FROM " . SOCIALMEDIA_TBL . " WHERE status <> 'DELETE' $search ";
    ...
    $position_qry_string .= "&cname1=news_title&cname2=socialmedia_id";

    ?>

<div class="" align="right"><a href="<?php echo PAGE_CHANGE_NEWS_POSTION; ?>" class="newsPos"
        style="text-decoration: none;">Set News Position</a>
                                ...
                                <table width="100%" cellspacing="0" cellpadding="0">
                                    <tr>
                                        ...
                                        <?php if (intval($dA) > intval(0)) {?><td align="right"
                                            style="padding-right:10px;"><b>Total Records: <?php echo intval($dA); ?></b>
                                        </td><?php }?>
                                    </tr>
                                </table>
                            </td>
        ...
        ?>
            ...
            <tr>
                <td class="list_table" valign="top">
                    <input type="hidden" name="qryString" id="qryString"
                        value="<?php echo $position_qry_string; ?>&stop=1" style="width: 500px;" />
                    <table cellpadding="0" cellspacing="0" width="100%" border='0'>
                        <tr>
...
$CK_COUNTER = 0;
        ...
        foreach ($row as $rs) {
            ...
            $socialmedia_id = intval($rs['socialmedia_id']);
            ...
            ?>
                                    ...
                                    <li id="listItem_<?php echo $socialmedia_id; ?>">
                                        <table cellpadding="0" cellspacing="0" width="100%" border="0">
                                            <tr class="expiredCoupons trhover">
                                                ...
                                                <td align="center" width="5%">
                                                    <div id="INPROCESS_STATUS_1_<?php echo $socialmedia_id; ?>"
...
if ($DISPLAY_IMG != '') {
                ?>
                                                    <img src="<?php echo $DISPLAY_IMG; ?>" alt="" width="30"
                                                        height="30" />
                                                    <?php
}
            ?>
                                                ...
                                                <td align="center" width="15%">
                                                    <div id="INPROCESS_SETDISPLAY_1_<?php echo $socialmedia_id; ?>"
                                                        style="display: none;"></div>
                                                    <div id="INPROCESS_SETDISPLAY_2_<?php echo $socialmedia_id; ?>">
                                                        <a href="javascript:void(0);"
                                                            value="<?php echo $socialmedia_id; ?>"
                                                            myvalue="<?php echo $DISPLAYnowaction; ?>"
                                                            class="displayOnTop">
                                                            <img <?php
if ($display_on_top == intval(1)) {
                ?> src="<?php echo CMS_INCLUDES_ICON_RELATIVE_PATH; ?>set.png" title="Click To Unset"
                                                                alt="Click To Unset" />

                                                            <?php
} elseif ($display_on_top == intval(0)) {
                ?>
                                                            src="<?php echo CMS_INCLUDES_ICON_RELATIVE_PATH; ?>unset.png"
                                                            title="Click To Set" alt="Click To Set" />
                                                            <?php
}
            ?>
                                                        </a>
                                                ...
                                                <td align="center" width="10%">
                                                    <div id="INPROCESS_DELETE_1_<?php echo $socialmedia_id; ?>"
                                                        style="display: none;"></div>

                                                    <div id="INPROCESS_DELETE_2_<?php echo $socialmedia_id; ?>">
                                                        <a href="<?php echo PAGE_MAIN; ?>?id=<?php echo $socialmedia_id; ?>"
        ...
        ?>
            ...
            <tr>
                <td height="30" colspan="<?php echo $COLSPAN; ?>" class="txt1" style="padding-top:10px;" valign="top"
                    id="INPROCESS_DEL">
                    <input type="button" class="grey_btn delete_all" value="Delete Selected" id="delete_all"
                        disabled="" />
...
function deleteSelected()
{
    ...
    foreach ($exp as $chk) {
        ...
        ///$stmt = $dCON->prepare("DELETE FROM " . SOCIALMEDIA_TBL . "   WHERE section_id = ?");
        $stmt = $dCON->prepare("Update " . SOCIALMEDIA_TBL . "  set status='DELETE' WHERE socialmedia_id = ?");
...
function listNEWS_POSITIONWISE()
{
    ...
    $position_qry_string_display .= "con=" . SOCIALMEDIA_TBL;
    $position_qry_string_display .= "&cname1=news_title&cname2=socialmedia_id";
...
if (intval($dA) > intval(0)) {
    ...
    <form name="frmDel" id="frmDel" method="post" action="">
        ...
        <input type="hidden" name="qryStringDisplay" id="qryStringDisplay"
            value="<?php echo $position_qry_string_display; ?>&stop=1" style="width: 1100px;" />
