...
function removeImage()
{
    ...
    else
    {
        if(unlink(CMS_UPLOAD_FOLDER_RELATIVE_PATH . $FOLDER_NAME . "/" . $image_name)) 
        {
            ...
            $SQL = "";
            $SQL .= "UPDATE " . SIG24_TBL . " SET " ;
            $SQL .= " image_name = :image_name, ";
            $SQL .= " image_id = :image_id ";
            $SQL .= " WHERE sig24_id = :sig24_id ";
            ...
            $stk_upd->bindParam(":sig24_id", $imageId);
            $stk_upd->execute();
...
function removeFypeFILE()
{
    ...
    {
        //delete image
        if(unlink(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_DRAFT_BEST_PRACTICES . "/" . FLD_DRAFT_BEST_PRACTICES_FILE . "/" . $image_name)) 
        {
            echo "1~~~Deleted";
        } 
        else 
    ...
    {
        $IMG = getDetails(DRAFT_BEST_PRACTICES_TBL, 'file_name', "draft_id","$ID",'=', '', '' , "");  
         
        unlink(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_DRAFT_BEST_PRACTICES . "/" . FLD_DRAFT_BEST_PRACTICES_FILE . "/" . $IMG);
                  
        $stk_del = $dCON->prepare("update " . DRAFT_BEST_PRACTICES_TBL . " set file_name ='' WHERE draft_id = :draft_id " );
        $stk_del->bindParam(":draft_id", $ID);
        $stk_del->execute();
        $stk_del->closeCursor();
...
function saveData()
{
    ...
    $member_since_date = trustme($_REQUEST['member_since_date']);
    $member_since_date_array = explode("-", $member_since_date);
    $member_since_date = $member_since_date_array[2] . "-" . $member_since_date_array[1] . "-" . $member_since_date_array[0];   
    ...
    $url_key = filterString($company_name);
    ...
    $meta_title = trim($meta_title) == '' ? trim($company_name) : trim($meta_title);
    $meta_keyword = trim($meta_keyword) == '' ? trim($company_name) : trim($meta_keyword);
    $meta_description = trim($meta_description) == '' ? trim($company_name) : trim($meta_description);
    ...
    $FOLDER_NAME = "";
    $FOLDER_NAME = CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_SIG24 . "/";
	
    
    if($con == "add")
    {
        $CHK = checkDuplicate(SIG24_TBL,"company_name","$company_name","=","");
        //echo $section_name;       
                 
        if( intval($CHK) == intval(0) )
        {
            
            $MAX_ID = getMaxId(SIG24_TBL,"sig24_id");
            $MAX_POS = getMaxPosition(SIG24_TBL,"position","","","=");
            $MY_URLKEY = getURLKEY(SIG24_TBL,$url_key,$company_name,"","","",$MAX_ID,"");
            
            if($image != "")
            {
                $title_filter = filterString($company_name);
            ...
            $sql  = "";
            $sql .= " INSERT INTO " . SIG24_TBL . " SET ";
            $sql .= " sig24_id = :sig24_id, ";
            $sql .= " company_name = :company_name, ";
            $sql .= " url = :url, ";
            $sql .= " brief_description = :brief_description, ";     
            $sql .= " image_name = :image_name, ";
            $sql .= " image_id = :image_id, ";
            $sql .= " member_since_date = :member_since_date, ";
    ...
    else if($con == "modify")
    {
        ...
        $CHK = checkDuplicate(SIG24_TBL,"company_name~~~sig24_id","$company_name~~~$id","=~~~<>","");
        
        
        if( intval($CHK) == intval(0) )
        { 
            $MY_URLKEY = getURLKEY(SIG24_TBL,$url_key,$company_name,"sig24_id",$id,"<>",$id); 
...
function listData()
{
    ...
    $company_name = trustme($_REQUEST['search_company_name']);
    ...
    if($company_name != "")
    {
        $search .= " and company_name like :company_name ";
    }
    ...
    $SQL = "";
    $SQL .= " SELECT A.* ";     
    $SQL .= " FROM " . SIG24_TBL . " as A WHERE  A.status <> 'DELETE' $search ORDER BY position, sig24_id desc "; 
    
    $SQL_PG = " SELECT COUNT(*) AS CT FROM " . SIG24_TBL . " WHERE status <> 'DELETE' $search ";
    ...
    if($company_name != "")
    {
        $stmt1->bindParam(":company_name", $ntitle);
        $ntitle = "%{$company_name}%";
    }
    if(trim($search_fdate) != "" && trim($search_tdate) != "" ){ 
    ...
    if($company_name != "")
    {
        $stmt2->bindParam(":company_name", $ntitle);
        $ntitle = "%{$company_name}%";
    }
    ...
    $position_qry_string = "";
    $position_qry_string .= "con=".SIG24_TBL;
    $position_qry_string .= "&cname1=company_name&cname2=sig24_id";
                ...
                <td class="list_table" valign="top">
                    ...
                    <table cellpadding="0" cellspacing="0" width="100%" border='0'>
                        <tr>
                            ...
                            <th width="55%" align="left">Company Name</th>                                                 
                            <th width="10%" style="text-align: center;">Logo</th>
                            <th width="5%" style="text-align: center;">Position</th>                            
                            ...
                            <td colspan="7" style="padding:0px; border-bottom:0px;">                               
                                <ul id="test-list" style="padding: 0px;list-style: none;">                      
                                    ...
                                    foreach($row as $rs)
                                    {
                                        ...
                                        $DISPLAY_IMG = "";
                                        $R200_IMG_EXIST = chkImageExists(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_SIG24 . "/R200-" . $image_name);
                                        //$MAIN_IMG_EXIST = chkImageExists(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_SIG24 . "/" . $image_name);
                                        
                                        if( intval($R200_IMG_EXIST) == intval(1) )
                                        {
                                            $DISPLAY_IMG = CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_SIG24 . "/R200-" . $image_name;
                                        }
                                        ?>     
                                        
                                        <li id="listItem_<?php echo $sig24_id; ?>"> 
                                            <table cellpadding="0" cellspacing="0" width="100%" border="0">
                                                <tr class="expiredCoupons trhover">
                                        
                                                    <td width="5%"> 
                                                        <input type="checkbox" class="cb-element" name="chk[]" value="<?php echo $sig24_id; ?>" />
                                                    </td>
                                                    
                                                    <td align="center" width="5%">
                                                       <div id="INPROCESS_STATUS_1_<?php echo $sig24_id; ?>" style="display: none;"></div>
                                                        <div id="INPROCESS_STATUS_2_<?php echo $sig24_id; ?>"  >
                                                            <a href="javascript:void(0);" value="<?php  echo $sig24_id; ?>" myvalue="<?php echo $STATUSnowaction; ?>" class="setStatus">
                                                                <img 
                                                                <?php 
                                                    ...
                                                    <td align="center" width="10%">
                                                       <div id="INPROCESS_DELETE_1_<?php echo $sig24_id; ?>" style="display: none;"></div>
                                                       
                                                       <div id="INPROCESS_DELETE_2_<?php echo $sig24_id; ?>">
                                                            <a href="<?php echo PAGE_MAIN; ?>?id=<?php echo $sig24_id; ?>" class="modifyData img_btn">
                                                            ...
                                                            {
                                                            ?>
                                                                <a href="javascript:void(0);" value="<?php  echo $sig24_id; ?>" class="deleteData img_btn">
...
function deleteSelected()
{
    ...
    foreach($exp as $chk)
    {
        ...
        ///$stmt = $dCON->prepare("DELETE FROM " . SIG24_TBL . "   WHERE section_id = ?");
        $stmt = $dCON->prepare("Update " . SIG24_TBL . "  set status='DELETE' WHERE sig24_id = ?");
...
function saveINTRO()
{
    ...
    if($con == "add")
    {
        ...
        $MAX_ID = getMaxId(SIG24_INTRO_TBL,"intro_id");
                          
        $sql  = "";
        $sql .= " INSERT INTO " . SIG24_INTRO_TBL . " SET ";
    ...
    else if($con == "modify")
    {
        ...
        $sql  = "";
        $sql .= " UPDATE " . SIG24_INTRO_TBL . " SET ";
        $sql .= " intro_content = :intro_content, ";
        $sql .= " update_ip = :update_ip, ";
        $sql .= " update_time = :update_time, "; 
        $sql .= " update_by = :update_by "; 
