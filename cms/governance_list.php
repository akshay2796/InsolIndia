...
$QRY = "";
$QRY = " select * from ".GOVERNANCE_TYPE_TBL." AS s where s.status= 'ACTIVE' and type_id=:type_id";
//echo "==".$QRY.$atid;
$stmSec = $dCON->prepare($QRY);
$stmSec->bindParam(":type_id",$ctid);
$stmSec->execute();
...
{ 
   $subTYPE = getDetails(GOVERNANCE_SUBTYPE_TBL, '*', "status~~~type_id","ACTIVE~~~$ctid",'=~~~=', '', '' , "");
   $subCOUNT = count($subTYPE); // for submenu filter 
   
   $type_name = htmlentities(stripslashes($rowSec[0]['type_name']));
}
?> 
...
$(document).ready(function(){
     ...
     $.fn.listData = function() {
        ...
        $.ajax({
            ...
            data: "type=listData&ctid=" + "<?php echo $ctid; ?>&" + search_val + "&page=" + pageNo,
            beforeSend: function(){  
    ...
    function loadUrl()
    {
        ...
        var search_governance_post = queryString['search_governance_post'] || "";
        var search_governance_name = queryString['search_governance_name'] || "";
        var search_subtype_id = queryString['search_subtype_id'] || "";
        
        var search_val = "search_governance_post=" + search_governance_post;
        search_val = search_val + "&search_governance_name=" + search_governance_name;
        search_val = search_val + "&search_subtype_id=" + search_subtype_id;
    ...
    $("#reset").click(function(){ 
     ...
     $.fn.loadSamePageViaAjax = function() {
        ...
        $("#search_governance_post").val("");
        $("#search_governance_name").val("");
        $("#search_subtype_id").val("");
...
<h1>Governance List <span> - <?php echo $type_name;?></span> <div class="addProductBox"><a href="<?php echo PAGE_MAIN;?>?ctid=<?php echo $ctid;?>" class="backBtn">Add New</a></div></h1>
    ...
    <div class="containerPad">
        <form id="frm" name="frm" method="post" action="" enctype="multipart/form-data">
            <div class="fullWidth noGap">
                <?php
                    if($subCOUNT > intval(0))
                    {
                ?>
                        <div class="width4">
                        	<label class="mainLabel">Governance Subtype</label>
                            <select name="search_subtype_id" id="search_subtype_id" class="selectBox">
                                <option value="">--Select Submenu--</option>
                                <?php           
                                  
                                if ( count($subCOUNT) > intval(0) )
                                {                         
                                    foreach($subTYPE as $rsTYPE)
                                    {
                                ?>
                                        <option value="<?php echo intval($rsTYPE['subtype_id']); ?>" ><?php echo htmlentities(stripslashes($rsTYPE['subtype_name']));?></option>
                                <?php
                                    }
                                }
                                ?> 
                ...
                <div class="width4">
                    ...
                    <input type="text" class="txtBox" name="search_governance_name" id="search_governance_name" value="" maxlength="100"/>
                </div>
                 
                 <div class="width4">
                    <label class="mainLabel">Post <span></span></label>
                    <input type="text" class="txtBox" value="" name="search_governance_post" id="search_governance_post" AUTOCOMPLETE = "OFF"/>
                </div>
