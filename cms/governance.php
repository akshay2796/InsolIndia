...
define("PAGE_LIST","governance_list.php");

if( !is_dir(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_GOVERNANCE))
{
    $mask=umask(0);
    mkdir(CMS_UPLOAD_FOLDER_RELATIVE_PATH .  FLD_GOVERNANCE, 0777); 
    umask($mask);      
}


$governance_id = intval($_REQUEST['id']);
if( intval($governance_id) > intval(0) )
{   
    $con = "modify"; 
    $id = intval($governance_id);
    
    $stmt = $dCON->prepare(" SELECT * FROM " . GOVERNANCE_TBL . " WHERE governance_id = ? ");
    $stmt->bindParam(1, $governance_id);
    $stmt->execute();
    $row_stmt = $stmt->fetchAll();
    $stmt->closeCursor();
    
    $governance_id = intval($row_stmt[0]['governance_id']);
    $type_id = intval(stripslashes($row_stmt[0]['type_id']));
    $subtype_id = intval(stripslashes($row_stmt[0]['subtype_id']));
    $governance_name = htmlentities(stripslashes($row_stmt[0]['governance_name'])); 
    $governance_email = htmlentities(stripslashes($row_stmt[0]['governance_email']));
    $governance_post = htmlentities(stripslashes($row_stmt[0]['governance_post']));
    $governance_profession = htmlentities(stripslashes($row_stmt[0]['governance_profession']));
    $governance_profile = stripslashes($row_stmt[0]['governance_profile']); 
...
$(document).ready(function(){
     ...
     $('#type_id').on('change',function(){
        ...
        if(typeID){
            $.ajax({
                type:'POST',
                url:'<?php echo PAGE_AJAX; ?>',
                data:'type=findSUBTYPE&type_id='+typeID,
            ...
            $('#subtype_id').html('<option value="">Select Type first</option>'); 
        }
    });
    
    $.validator.addMethod('chkDataText', function (data){
        if( $.trim(CKEDITOR.instances.governance_profile.getData()) == "" )
        {
            return 0;
        }
        else
    ...
    $("#frm").validate({
        ...
        submitHandler: function() {
            ...
            var dcontent = escape(CKEDITOR.instances.governance_profile.getData()); 
               ...
               success: function(msg){                   
                   ...
                   setTimeout(function(){
                        ...
                        if( parseInt(spl_txt[1]) == parseInt(1) )
                        {
                            if(cond =="modify")
                            {
                                window.location.href = "<?php echo PAGE_LIST; ?>?ctid=<?php echo $type_id;?>"; 
                            }
                            else
                            {
                                window.location.href = "<?php echo PAGE_MAIN; ?>?ctid="+$("#type_id").val(); 
                            }
                        }
    ...
    $("#cancel").live("click", function(){    
        ...
        {
        ?>
            location.href='<?php echo PAGE_LIST; ?>?ctid=<?php echo $type_id;?>';
        <?php
        }
        else
        {
    ...
    <div class="containerPad expendableBox1">
        <div class="fullWidth">                    	
            <div class="width4">
                ...
                <select name="type_id" id="type_id" class="selectBox">
                    ...
                    <?php           
                    $rowCT = getDetails(GOVERNANCE_TYPE_TBL, '*', "status","ACTIVE",'=', 'type_name', '' , "");   
                    if ( count($rowCT) > intval(0) )
                    {                         
                        foreach($rowCT as $rsCT)
                        {
                    ?>
                            <option value="<?php echo intval($rsCT['type_id']); ?>" <?php if( intval($rsCT['type_id']) == intval($type_id) ) { echo " selected "; } ?> ><?php echo trim(stripslashes($rsCT['type_name']));?></option>
                    <?php
                        }
                    }
                    ?> 
            ...
            <div class="width4">
                    <label class="mainLabel">Governance Sub Type </label>
                        <select class="selectBox" name="subtype_id" id="subtype_id" data-placeholder="Select SubType" class="form-group" >
                             
                            <?php 
                                if($subtype_id > intval(0))
                                {
                                    $subNAME = getDetails(GOVERNANCE_SUBTYPE_TBL, 'subtype_name', "status~~~subtype_id","ACTIVE~~~$subtype_id",'=~~~=', '', '' , "");
                                    ?>
                                    <option value="<?php echo $subtype_id; ?>"><?php echo $subNAME; ?> </option> 
                                    <?php
                                }else{
                                    ?>
            ...
            <div class="width4">
                <label class="mainLabel">Member Name <span>*</span></label>
                <input type="text" class="txtBox" value="<?php echo $governance_name;?>" name="governance_name" id="governance_name" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
            <div class="width4">
                <label class="mainLabel">Post </label>
                <input type="text" class="txtBox" value="<?php echo $governance_post;?>" name="governance_post" id="governance_post" AUTOCOMPLETE = "OFF" maxlength="200"/>
            </div>                       
        </div>
        ...
        <div class="fullWidth">
            <div class="">
                ...
                <input type="text" class="txtBox" value="<?php echo $governance_profession;?>" name="governance_profession" id="governance_profession" AUTOCOMPLETE = "OFF" maxlength="300"/>
            </div> 
        </div>
        <div class="fullWidth">
            <div class="width4">
                <label class="mainLabel">Email <span>*</span></label>
                <input type="text" class="txtBox" value="<?php echo $governance_email;?>" name="governance_email" id="governance_email" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
        </div>
        ...
        <div class="fullWidth div_content validateMsg">
            <textarea name="governance_profile" id="governance_profile" style="width:90%; height:230px; float:left;"><?php echo $governance_profile; ?></textarea>
            <script>
            CKEDITOR.replace("governance_profile",
                {
                    enterMode: 2,
