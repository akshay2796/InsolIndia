...
$event_joiner_id = intval($_REQUEST['eventjoinerid']);
...
$(document).ready(function(){     
    $.fn.listData = function() {
        ...
        $.ajax({
            ...
            data: "type=listData&event_joiner_id=<?php echo $event_joiner_id; ?>&" + search_val + "&page=" + pageNo,
            beforeSend: function(){  
...
function ChangeStatus(event_joiner_id,status) {

    $.ajax({
        type: "POST",
        url: "<?php echo PAGE_AJAX; ?>",
        data: "type=changeStatus&ID=" + event_joiner_id +"&status="+ status,
        beforeSend: function(){
            $("#INPROCESS_DELETE_2_" + ID).hide();
            $("#INPROCESS_DELETE_1_" + ID).show();
            $("#INPROCESS_DELETE_1_" + ID).html("<img src='<?php echo CMS_INCLUDES_IMAGES_RELATIVE_PATH; ?>loader-small.gif' border='0' />");
...
<h1>Event Joiner List</h1>
    
<div class="addWrapper" <?php if($event_joiner_id > intval(0)) { echo 'style="display:none;"'; } ?>>
