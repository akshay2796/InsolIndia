...
define("PAGE_LIST","judges_advisory_list.php");

if( !is_dir(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_JUDGES_ADVISORY_ROUNDTABLE))
{
    $mask=umask(0);
    mkdir(CMS_UPLOAD_FOLDER_RELATIVE_PATH .  FLD_JUDGES_ADVISORY_ROUNDTABLE, 0777); 
    umask($mask);      
}


$judges_ar_id = intval($_REQUEST['id']);
if( intval($judges_ar_id) > intval(0) )
{   
    $con = "modify"; 
    $id = intval($judges_ar_id);
    
    $stmt = $dCON->prepare(" SELECT * FROM " . JUDGES_ADVISORY_ROUNDTABLE_TBL . " WHERE judges_ar_id = ? ");
    $stmt->bindParam(1, $judges_ar_id);
    $stmt->execute();
    $row_stmt = $stmt->fetchAll();
    $stmt->closeCursor();
    
    $judges_ar_id = intval($row_stmt[0]['judges_ar_id']);
    $judges_ar_name = htmlentities(stripslashes($row_stmt[0]['judges_ar_name'])); 
    $judges_ar_email = htmlentities(stripslashes($row_stmt[0]['judges_ar_email']));
    $judges_ar_post = htmlentities(stripslashes($row_stmt[0]['judges_ar_post']));
    $judges_ar_profession = htmlentities(stripslashes($row_stmt[0]['judges_ar_profession']));
    $judges_ar_profile = stripslashes($row_stmt[0]['judges_ar_profile']); 
...
$(document).ready(function(){
    ...
    $.validator.addMethod('chkDataText', function (data){
        if( $.trim(CKEDITOR.instances.judges_ar_profile.getData()) == "" )
        {
            return 0;
        }
        else
    ...
    $("#frm").validate({
        ...
        ignore:[],
        rules: {
            judges_ar_name: "required"
           // judges_ar_post: "required"
        },
        messages: {
            judges_ar_name: ""
            //judges_ar_post: ""      
        ...
        submitHandler: function() {
            ...
            var dcontent = escape(CKEDITOR.instances.judges_ar_profile.getData()); 
...
<h1>Judges Advisory Roundtable <div class="addProductBox"><a href="<?php echo PAGE_LIST;?>" class="backBtn">Go To List</a></div></h1>
    ...
    <div class="containerPad expendableBox1">
        <div class="fullWidth">                    	
            <div class="width4">
                ...
                <input type="text" class="txtBox" value="<?php echo $judges_ar_name;?>" name="judges_ar_name" id="judges_ar_name" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
            <div class="width4">
                <label class="mainLabel">Post </label>
                <input type="text" class="txtBox" value="<?php echo $judges_ar_post;?>" name="judges_ar_post" id="judges_ar_post" AUTOCOMPLETE = "OFF" maxlength="200"/>
            </div>
            <div class="width2">
                <label class="mainLabel">Profession <span></span><span></span></label>
                <input type="text" class="txtBox" value="<?php echo $judges_ar_profession;?>" name="judges_ar_profession" id="judges_ar_profession" AUTOCOMPLETE = "OFF" maxlength="300"/>
            </div>            
        </div>
        <div class="fullWidth">
            <div class="width4">
                <label class="mainLabel">Email </label>
                <input type="text" class="txtBox" value="<?php echo $judges_ar_email;?>" name="judges_ar_email" id="judges_ar_email" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
        </div>
        ...
        <div class="fullWidth div_content validateMsg">
            <textarea name="judges_ar_profile" id="judges_ar_profile" style="width:90%; height:230px; float:left;"><?php echo $judges_ar_profile; ?></textarea>
            <script>
            CKEDITOR.replace("judges_ar_profile",
                {
                    enterMode: 2,
