...
define("PAGE_LIST","committee_list.php");

if( !is_dir(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_COMMITTEES))
{
    $mask=umask(0);
    mkdir(CMS_UPLOAD_FOLDER_RELATIVE_PATH .  FLD_COMMITTEES, 0777); 
    umask($mask);      
}


$committee_id = intval($_REQUEST['id']);
if( intval($committee_id) > intval(0) )
{   
    $con = "modify"; 
    $id = intval($committee_id);
    
    $stmt = $dCON->prepare(" SELECT * FROM " . COMMITTEES_TBL . " WHERE committee_id = ? ");
    $stmt->bindParam(1, $committee_id);
    $stmt->execute();
    $row_stmt = $stmt->fetchAll();
    $stmt->closeCursor();
    
    $committee_id = intval($row_stmt[0]['committee_id']);
    $committee_type_id = htmlentities(stripslashes($row_stmt[0]['committee_type_id']));
    $committee_name = htmlentities(stripslashes($row_stmt[0]['committee_name'])); 
    $committee_email = htmlentities(stripslashes($row_stmt[0]['committee_email']));
    $committee_post = htmlentities(stripslashes($row_stmt[0]['committee_post']));
    $committee_profession = htmlentities(stripslashes($row_stmt[0]['committee_profession']));
    $committee_profile = stripslashes($row_stmt[0]['committee_profile']); 
...
$(document).ready(function(){
    ...
    $.validator.addMethod('chkDataText', function (data){
        if( $.trim(CKEDITOR.instances.committee_profile.getData()) == "" )
        {
            return 0;
        }
        else
    ...
    $("#frm").validate({
        ...
        submitHandler: function() {
            ...
            var dcontent = escape(CKEDITOR.instances.committee_profile.getData()); 
               ...
               success: function(msg){                   
                   ...
                   setTimeout(function(){
                        ...
                        if( parseInt(spl_txt[1]) == parseInt(1) )
                        {
                            if(cond =="modify")
                            {
                                window.location.href = "<?php echo PAGE_LIST; ?>?ctid=<?php echo $committee_type_id;?>"; 
                            }
                            else
                            {
                                window.location.href = "<?php echo PAGE_MAIN; ?>?ctid="+$("#committee_type_id").val(); 
                            }
                        }
    ...
    $("#cancel").live("click", function(){    
        ...
        {
        ?>
            location.href='<?php echo PAGE_LIST; ?>?ctid=<?php echo $committee_type_id;?>';
        <?php
        }
        else
        {
...
<div class="addWrapper master">
    ...
    <div class="containerPad expendableBox1">
        <div class="fullWidth">                    	
            <div class="width4">
                <label class="mainLabel">Member Type <span>*</span></label>
                <select name="committee_type_id" id="committee_type_id" class="selectBox">
                    <option value="">--Select--</option>
                    <?php           
                    $rowCT = getDetails(COMMITTEES_TYPE_TBL, '*', "status","ACTIVE",'=', 'type_name', '' , "");   
                    if ( count($rowCT) > intval(0) )
                    {                         
                        foreach($rowCT as $rsCT)
                        {
                    ?>
                            <option value="<?php echo intval($rsCT['type_id']); ?>" <?php if( intval($rsCT['type_id']) == intval($committee_type_id) ) { echo " selected "; } ?> ><?php echo trim(stripslashes($rsCT['type_name']));?></option>
                    <?php
                        }
                    }
                    ?> 
            ...
            <div class="width4">
                ...
                <input type="text" class="txtBox" value="<?php echo $committee_name;?>" name="committee_name" id="committee_name" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
            <div class="width4">
                <label class="mainLabel">Post </label>
                <input type="text" class="txtBox" value="<?php echo $committee_post;?>" name="committee_post" id="committee_post" AUTOCOMPLETE = "OFF" maxlength="200"/>
            </div>                       
        </div>
        ...
        <div class="fullWidth">
            <div class="">
                ...
                <input type="text" class="txtBox" value="<?php echo $committee_profession;?>" name="committee_profession" id="committee_profession" AUTOCOMPLETE = "OFF" maxlength="300"/>
            </div> 
        </div>
        <div class="fullWidth">
            <div class="width4">
                <label class="mainLabel">Email </label>
                <input type="text" class="txtBox" value="<?php echo $committee_email;?>" name="committee_email" id="committee_email" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
        </div>
        ...
        <div class="fullWidth div_content validateMsg">
            <textarea name="committee_profile" id="committee_profile" style="width:90%; height:230px; float:left;"><?php echo $committee_profile; ?></textarea>
            <script>
            CKEDITOR.replace("committee_profile",
                {
                    enterMode: 2,
