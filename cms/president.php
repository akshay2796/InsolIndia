...
if( !is_dir(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_PRESIDENT))
{
    $mask=umask(0);
    mkdir(CMS_UPLOAD_FOLDER_RELATIVE_PATH .  FLD_PRESIDENT, 0777); 
    umask($mask);      
}


$president_id = intval($_REQUEST['id']);
if( intval($president_id) > intval(0) )
{   
    $con = "modify"; 
    $id = intval($president_id);
    
    $stmt = $dCON->prepare(" SELECT * FROM " . PRESIDENT_TBL . " WHERE president_id = ? ");
    $stmt->bindParam(1, $president_id);
    $stmt->execute();
    $row_stmt = $stmt->fetchAll();
    $stmt->closeCursor();
    
    $president_id = intval($row_stmt[0]['president_id']);
    $president_name = htmlentities(stripslashes($row_stmt[0]['president_name'])); 
    $president_profession = htmlentities(stripslashes($row_stmt[0]['president_profession']));
    $president_profile = stripslashes($row_stmt[0]['president_profile']); 
...
$(document).ready(function(){
    ...
    $.validator.addMethod('chkDataText', function (data){
        if( $.trim(CKEDITOR.instances.president_profile.getData()) == "" )
        {
            return 0;
        }
        else
    ...
    $("#frm").validate({
        ...
        submitHandler: function() {
            ...
            var dcontent = escape(CKEDITOR.instances.president_profile.getData()); 
...
<h1>President <div class="addProductBox"><a href="<?php echo PAGE_LIST;?>" class="backBtn">Go To List</a></div></h1>
    ...
    <div class="containerPad expendableBox1">
        <div class="fullWidth">                    	
            <div class="width4">
                ...
                <input type="text" class="txtBox" value="<?php echo $president_name;?>" name="president_name" id="president_name" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
            <div class="width2">
                <label class="mainLabel">Profession <span>*</span><span></span></label>
                <input type="text" class="txtBox" value="<?php echo $president_profession;?>" name="president_profession" id="president_profession" AUTOCOMPLETE = "OFF" maxlength="300"/>
            </div>            
        </div>
        ...
        <div class="fullWidth div_content validateMsg">
            <textarea name="president_profile" id="president_profile" style="width:90%; height:230px; float:left;"><?php echo $president_profile; ?></textarea>
            <script>
            CKEDITOR.replace("president_profile",
                {
                    enterMode: 2,
