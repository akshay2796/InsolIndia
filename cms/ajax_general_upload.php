                        ...
                        <td class="main_heading">
                            <table width="100%" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <?php if(intval($dA) > intval(0)) {?><td align="right" style="padding-right:10px;"><b>Total Record<?php if(intval($dA) > intval(1)){ echo "s"; } ?>: <?php echo intval($dA);?></b></td><?php } ?>
                                 </tr>
                            </table>
                        </td>
                      </tr> 
                            ...
                            <tr class="expiredCoupons trhover" >
                                ...
                                <td align="center" >                           
                                   ...
                                   <div id="INPROCESS_DELETE_2_<?php echo $upload_id; ?>"  >
                                        ...
                                        <a href="<?php echo PAGE_PREVIEW;?>?pid=<?php echo $upload_id; ?>" class="img_btn view_detail" title="View Details"><img border="0" src="<?php echo CMS_INCLUDES_ICON_RELATIVE_PATH; ?>view.png" class="cmsIcon" alt="View Details" ></a></span>
                                        
                                        <a href="<?php echo PAGE_MAIN;?>?con=modify&id=<?php echo $upload_id; ?>" class="img_btn viewDetail" title="Modify">
                                        ...
                                        {
                                        ?>
                                            <a href="javascript:void(0);" value="<?php  echo $upload_id; ?>" class="deleteData img_btn">
...
function sendNewsletter()
{
    ...
    $eventName = trustme($_REQUEST['eventName']);
            $eventVenue = trustme($_REQUEST['eventVenue']);
            $eventSDate = trustme($_REQUEST['eventSDate']);
            $eventSTime = trustme($_REQUEST['eventSTime']);
            $eventEDate = trustme($_REQUEST['eventEDate']);
            $eventETime = trustme($_REQUEST['eventETime']);
            
            $time=strtotime($eventSDate);
                    $month=date("m",$time);
                    $year=date("Y",$time);
                    $day=date('d',$time);
                    $time2=strtotime($eventEDate);
                    $month2=date("m",$time2);
                    $year2=date("Y",$time2);
                    $day2=date('d',$time2);
                    
                    $Ftime=strtotime($eventSTime);
                    $hourd = date("H",$Ftime);
                    $mind = date("i",$Ftime);
                    $secd = date("s",$Ftime);
                    $am_pm = date("A",$Ftime);
                    
                    $Ftime2=strtotime($eventETime);
                    $hourd2 = date("H",$Ftime2);
                    $mind2 = date("i",$Ftime2);
                    $secd2 = date("s",$Ftime2);
                    $am_pm2 = date("A",$Ftime2);
            $array_ics = array(
                        "year" => "$year",
                        "month" => "$month",
                        "day" => "$day",
        ...
        if(count($tsGET)>intval(0))
        {
            ...
            foreach($tsGET as $rs)
            {
               ...
               // $MAIL_FORMAT = str_ireplace("%memail%", $email, $MAIL_FORMAT);
                ...
                {
            $RES_M = MailObject($TO_EMAIL,$FROM_EMAIL,"", "", "$general_newsletter_subject", $MAIL_FORMAT, "","",$array_ics);
                }else{
                    $RES_M = MailObject($TO_EMAIL,$FROM_EMAIL,"", "", "$general_newsletter_subject", $MAIL_FORMAT, "","","no_ics");
                }
                
                // $RES_M = MailObject($TO_EMAIL,$FROM_EMAIL,"", "", "$general_newsletter_subject", $MAIL_FORMAT, "");
     ...
     if(intval(count($governance_id_array)) > intval(0))
     {
        ...
        foreach($governance_id_array as $governance_id)
        {
            ...
            foreach($esGET as $emailVAL){
                ...
                {
            $RES_GOV = MailObject($TO_EMAIL,$FROM_EMAIL,"", "", "$general_newsletter_subject", $MAIL_FORMAT, "","",$array_ics);
                }else{
                    $RES_GOV = MailObject($TO_EMAIL,$FROM_EMAIL,"", "", "$general_newsletter_subject", $MAIL_FORMAT, "","","no_ics"); 
                }
                
                // $RES_GOV = MailObject($TO_EMAIL,$FROM_EMAIL,"", "", "$general_newsletter_subject", $MAIL_FORMAT, ""); 
    ...
    if(intval($send_to_insol_member) == intval(1))
    {
        ...
        if(count($rsGET)>intval(0))
        {
            ...
            $_SESSION['group_members_insol_member'] = $RES_MEMBER_CT;
        }
   ...
   // to imnsol members starts ============================================
    ...
    if(intval($send_to_all_insol) == intval(1))
    {
        ...
        if(count($rsGET)>intval(0))
        {
            ...
            $_SESSION['group_members_all_insol_member'] = $RES_MEMBER_CT;
        }
