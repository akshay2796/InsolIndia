...
define("PAGE_MAIN", "governance_type.php");
define("PAGE_AJAX", "ajax_governance_type.php");
...
function saveData()
{
    ...
    $type_description = trustyou($_REQUEST['type_description']);
    ...
    if ($con == "add") {
        $CHK = checkDuplicate(GOVERNANCE_TYPE_TBL, "type_name", "$type_name", "=", "");
        //echo $type_name;
...
function listData()
{
    ...
    $SQL = "";
    $SQL .= " SELECT * FROM " . GOVERNANCE_TYPE_TBL . " as B ";
    ...
    $position_qry_string = "";
    $position_qry_string .= "con=" . GOVERNANCE_TYPE_TBL;
...
if (trim($status) == 'ACTIVE') {
                ?>
                                    <a href="javascript:void(0);" value="<?php echo $type_id; ?>"
...
function modifyData()
{
    ...
    $stmt = $dCON->prepare(" SELECT * FROM " . GOVERNANCE_TYPE_TBL . " WHERE type_id = ? ");
    ...
    $type_description = (stripslashes($row_stmt[0]['type_description']));
    ...
    echo "~~~$type_id~~~$type_name~~~$type_description~~~$meta_title~~~$meta_keyword~~~$meta_description~~~";
...
function deleteSelected()
{
    ...
    foreach ($exp as $chk) {
        ...
        $stmt = $dCON->prepare("DELETE FROM " . GOVERNANCE_TYPE_TBL . "   WHERE type_id = ?");
...
function setStatus()
{
    ...
    $STR = "";
    $STR .= " UPDATE  " . GOVERNANCE_TYPE_TBL . "  SET ";
    $STR .= " status = :status, ";
...
function chkFor($ID)
{
    ...
    $SQL .= " SELECT COUNT(*) AS CT FROM " . GOVERNANCE_TBL . " WHERE type_id = ? AND status <> 'DELETE' ";
    //$SQL .= " UNION ";
