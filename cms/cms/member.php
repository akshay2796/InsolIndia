...
define("PAGE_MAIN","member.php");	
define("PAGE_AJAX","ajax_member.php");
define("PAGE_LIST","member_list.php");

if( !is_dir(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_MEMBER_DIRECTORY))
{
    $mask=umask(0);
    mkdir(CMS_UPLOAD_FOLDER_RELATIVE_PATH .  FLD_MEMBER_DIRECTORY, 0777); 
    umask($mask);      
}
...
{   
    ...
    $stmt = $dCON->prepare(" SELECT * FROM " . MEMBER_TBL . " WHERE member_id = ? ");
    $stmt->bindParam(1, $member_id);
    $stmt->execute();
    $row_stmt = $stmt->fetchAll();
    $stmt->closeCursor();
    
    $member_id = intval($row_stmt[0]['member_id']);
    $member_type_id = htmlentities(stripslashes($row_stmt[0]['member_type_id']));
    $member_name = htmlentities(stripslashes($row_stmt[0]['member_name'])); 
    $member_post = htmlentities(stripslashes($row_stmt[0]['member_post']));
    $member_profession = htmlentities(stripslashes($row_stmt[0]['member_profession']));
   // $member_profile = stripslashes($row_stmt[0]['member_profile']); 
    //$image_name = trim(stripslashes($row_stmt[0]['image_name']));  
    //$image_id = trim(stripslashes($row_stmt[0]['image_id']));   
    
    //$FOLDER_NAME = FLD_MEMBER_DIRECTORY; 
...
else
{
    ...
    $member_profession = "";    
    //$member_profile = ""; 
    $member_type_id = intval($_REQUEST['mtid']);  
...
$(document).ready(function(){
   ...
   /* $.validator.addMethod('chkDataText', function (data){
        if( $.trim(CKEDITOR.instances.member_profile.getData()) == "" )
        {
            return 0;
        }
        else
    ...
    $("#frm").validate({
        ...
        submitHandler: function() {
          ...
          /*  var ImgUploading = $('#checkImgUploadingProgress').val();
            ...
            var dcontent = escape(CKEDITOR.instances.member_profile.getData()); */
               ...
               success: function(msg){                   
                   ...
                   setTimeout(function(){
                        ...
                        if( parseInt(spl_txt[1]) == parseInt(1) )
                        {
                            if(cond =="modify")
                            {
                                window.location.href = "<?php echo PAGE_LIST; ?>?mtid=<?php echo $member_type_id;?>"; 
                            }
                            else
                            {
                                window.location.href = "<?php echo PAGE_MAIN; ?>?mtid="+$("#member_type_id").val(); 
                            }
                        }
    ...
    $("#cancel").live("click", function(){    
        ...
        {
        ?>
            location.href='<?php echo PAGE_LIST; ?>?mtid=<?php echo $member_type_id;?>';
        <?php
        }
        else
        {
    ...
    <div class="containerPad expendableBox1">
        <div class="fullWidth">                    	
            <div class="width4">
                ...
                <select name="member_type_id" id="member_type_id" class="selectBox">
                    ...
                    {                         
                        ...
                        {
                    ?>
                            <option value="<?php echo intval($rsCT['type_id']); ?>" <?php if( intval($rsCT['type_id']) == intval($member_type_id) ) { echo " selected "; } ?> ><?php echo trim(stripslashes($rsCT['type_name']));?></option>
                    <?php
                        }
                    }
                    ?> 
            ...
            <div class="width4">
                ...
                <input type="text" class="txtBox" value="<?php echo $member_name;?>" name="member_name" id="member_name" AUTOCOMPLETE = "OFF" maxlength="150"/>
            </div>
            <div class="width2">
                <label class="mainLabel">Profession <span></span><span></span></label>
                <input type="text" class="txtBox" value="<?php echo $member_profession;?>" name="member_profession" id="member_profession" AUTOCOMPLETE = "OFF" maxlength="300"/>
            </div>
       ...
       <!-- <div class="fullDivider noGap">
        ...
        <div class="fullWidth div_content validateMsg">
           <!-- <textarea name="member_profile" id="member_profile" style="width:90%; height:230px; float:left;"><?php echo $member_profile; ?></textarea> -->
            <script>
          /*  CKEDITOR.replace("member_profile",
                {
                    enterMode: 2,
