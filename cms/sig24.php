...
define("PAGE_LIST","sig24_list.php");

if( !is_dir(CMS_UPLOAD_FOLDER_RELATIVE_PATH . FLD_SIG24))
{
    $mask=umask(0);
    mkdir(CMS_UPLOAD_FOLDER_RELATIVE_PATH .  FLD_SIG24, 0777); 
    umask($mask);      
}


$sig24_id = intval($_REQUEST['id']);
if( intval($sig24_id) > intval(0) )
{   
    $con = "modify"; 
    $id = intval($sig24_id);
    
    $stmt = $dCON->prepare(" SELECT * FROM " . SIG24_TBL . " WHERE sig24_id = ? ");
    $stmt->bindParam(1, $sig24_id);
    $stmt->execute();
    $row_stmt = $stmt->fetchAll();
    $stmt->closeCursor();
    
    $sig24_id = intval($row_stmt[0]['sig24_id']);
    ...
    $member_since_date = stripslashes($row_stmt[0]['member_since_date']);
    
    if(trim($member_since_date) != "" && $member_since_date != "0000-00-00 00:00:00"){
        $member_since_date = date('d-m-Y' , strtotime($member_since_date));    
    }else{
        $member_since_date = "";
    }

	$METATITLE = trim(stripslashes($row_stmt[0]['meta_title'])); 
	$METADESCRIPTION = trim(stripslashes($row_stmt[0]['meta_description'])); 
	$METAKEYWORD = trim(stripslashes($row_stmt[0]['meta_keyword'])); 
...
$(document).ready(function(){
    ...
    $("#frm").validate({
        ...
        messages: {
            ...
            url: "",
            member_since_date: "",
            brief_description: "Description cannot be blank."      
        },
...
$(function() {
    ...
    $('#member_since_date').datepick({
        dateFormat: 'dd-mm-yyyy',
    	yearRange: '<?php echo date("Y", strtotime('-1 year')); ?>:<?php echo date("Y", strtotime('+10 year')); ?>'
    });  
...
<h1>SIG 24 <div class="addProductBox"><a href="<?php echo PAGE_LIST;?>" class="backBtn">Go To List</a></div></h1>
...
<div class="addWrapper master">
    ...
    <div class="containerPad expendableBox1">
        <div class="fullWidth">                    	
            <div class="width">
                <label class="mainLabel">Comapany Name <span>*</span></label>
                <input type="text" class="txtBox" value="<?php echo $company_name;?>" name="company_name" id="company_name" AUTOCOMPLETE = "OFF" maxlength="500"/>
            </div>
        </div>
        <div class="width4">
        	<label class="mainLabel">Member Since<span> *</span></label>
            <div class="txt3Box">
                <input type="text" class="titleTxt txtBox" name="member_since_date" id="member_since_date" style="width: 100px;" value="<?php echo $member_since_date; ?>" maxlength="12" placeholder="Date" style="width: 90px;" autocomplete="OFF" />&nbsp;&nbsp;
        ...
        <div class="fullWidth"> 
            <div class="width">
                ...
                <input type="text" class="txtBox" value="<?php echo $url;?>" name="url" id="url" AUTOCOMPLETE = "OFF" maxlength="500"/>
            </div>
        </div>
       
       
        <div class="fullDivider noGap">
            <div class="sml_heading ">Brief Description <span style="color: #FF0000;">*</span> </div>
        </div>
